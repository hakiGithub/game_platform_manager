...
- **部署扩展步骤（Deploy Extension Step）**
  - **定义**：部署流程中"部署动作完成 → 实例正式启动前"执行的一段声明式自动化（补丁替换 / 脚本执行）。由插件声明步骤清单（来源 + 目标路径 / 脚本内容或来源 / 执行位置），主应用部署管线按序阻塞执行，纳入部署日志与失败语义；声明接口可按实例配置动态计算步骤集。
  - **步骤属性**：致命性（默认致命——失败即部署失败、实例不交付，补丁步骤走 PatchInstallService 自带回滚）；执行位置（宿主机 SSH 为默认；容器内 docker exec 为步骤级可选项）。
  - **引入**：ADR-0029

- **部署扩展阶段（Deploy Extension Stage）**
  - **定义**：部署流水线新增的显式阶段，位于 DEPLOY 与 HEALTH_CHECK 之间。对"部署即起容器"的适配器（docker-compose）先停实例，执行扩展步骤后再进入既有启动与健康检查——首次启动即目标版本，无"先起错版本再重启"中间态。
  - **日志**：复用 `DeployService.taskStatusMap` 内存日志 + `DeployProgress.vue` 轮询展示，每步开始/成功/失败/耗时可见；部署日志落库不做（本期）。
  - **引入**：ADR-0029
...
- **版本目录（Version Catalog）**
  - **定义**：游戏侧可选择的版本清单：`版号 → 镜像 tag（可选，缺省沿用默认）+ 补丁集 + 脚本集 + 目标路径`。由插件经 ADR-0008 的部署配置声明体系声明（读时合并，不落库）；部署向导据此提供版本选择，选择结果随实例配置（configInfo）落库，重部署按同一配方重放。
  - **dnf-tw 现状**：上游镜像仅提供默认版本（qf1031），其余版本的目录条目只含补丁 + 脚本（无镜像 tag）。
  - **引入**：ADR-0029
