...
游戏侧可选择的版本清单：`版号 → 镜像 tag（可选，缺省沿用默认）+ 补丁集 + 脚本集 + 目标路径`。由插件经 ADR-0008 的部署配置声明体系声明（读时合并）；用户在部署向导中选择，选择结果随实例配置（configInfo）落库，重部署按同一配方重放。dnf-tw 现状：上游镜像只提供默认版本，其余版本靠补丁 + 脚本"改"出来。
