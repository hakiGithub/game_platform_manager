...
const tabs = [
  ...
  // 备份还原功能已临时下线（异步执行器不推进，见 .scratch/backlog/issues/01），
  // 恢复时还原该 tab 与对应 el-tab-pane 块
...
async function fetchInstanceDetail() {
  ...
  try {
    ...
    const mainPort =
      ...
      portConfig.gamePort ||
      portConfig.port ||
      (Object.keys(portConfig).length > 0
        ? portConfig[Object.keys(portConfig)[0]]
        : "");
...
async function fetchInstanceMetrics(isRefresh = false) {
  ...
  try {
    ...
    instanceInfo.value = {
      ...
      memory:
        data.memoryUsage != null ? data.memoryUsage : instanceInfo.value.memory,
      memoryUsageText:
        data.memoryUsageText || instanceInfo.value.memoryUsageText,
...
async function handleStop() {
  try {
    await ElMessageBox.confirm("确定要停止实例吗？", "确认操作", {
      type: "warning",
    });
    actionLoading.value = true;
...
async function handleRestart() {
  try {
    await ElMessageBox.confirm("确定要重启实例吗？", "确认操作", {
      type: "warning",
    });
    actionLoading.value = true;
...
const isLinuxGsmDocker = computed(
  () => instanceInfo.value.deployType === "linuxgsm-docker",
);
...
async function fetchConfig() {
  ...
  try {
    const data = await getInstanceConfig(instanceId.value, {
...
function scrollConsoleToBottom() {
  nextTick(() => {
    if (consoleContainerRef.value) {
      consoleContainerRef.value.scrollTop =
        consoleContainerRef.value.scrollHeight;
    }
  });
}
...
const runtimeMetadata = computed(
  () => instanceInfo.value.runtimeMetadata || null,
);
    ...
    <el-card v-loading="loading" class="info-card" shadow="never">
      <div class="command-identity">
        <div class="command-kicker">
          <span
            class="live-dot"
            :class="{ 'is-live': instanceInfo.status === 'running' }"
          />
          INSTANCE CONTROL ROOM
          <span class="command-channel"
            >INSTANCE /
            {{ String(instanceInfo.id || instanceId).padStart(2, "0") }}</span
          >
        </div>
      ...
      <div class="command-header">
        <div class="header-left">
          ...
          <div class="instance-mark">
            <img
              v-if="instanceInfo.iconUrl"
              :src="instanceInfo.iconUrl"
              :alt="instanceInfo.game"
              class="instance-mark-img"
            />
          ...
          <div class="instance-title">
            ...
            <div class="title-line">
              ...
              <el-tag
                :type="statusType(instanceInfo.status)"
                size="small"
                effect="dark"
              >
            ...
            <div class="instance-meta">
              <span
                ><el-icon><Grid /></el-icon> {{ instanceInfo.game }}</span
              >
              <span
                ><el-icon><Cpu /></el-icon>
                {{ getDeployTypeShort(instanceInfo.deployType) }}</span
              >
              <span
                ><el-icon><Monitor /></el-icon>
                {{ instanceInfo.hostName }}</span
              >
              <span
                ><el-icon><Link /></el-icon> {{ instanceInfo.ip }}:{{
                  instanceInfo.port
                }}</span
              >
            </div>
        ...
        <div class="command-state">
          ...
          <strong>{{
            instanceInfo.status === "running" ? "LIVE" : "STANDBY"
          }}</strong>
          <small>{{
            instanceInfo.status === "running"
              ? "接受实时遥测与运维指令"
      ...
      <div class="command-dashboard" aria-label="实例运行指标">
        <div class="command-stat command-stat--players">
          ...
          <strong
            >{{ instanceInfo.players || 0
            }}<small> / {{ instanceInfo.maxPlayers || 0 }}</small></strong
          >
          <div class="player-track">
            <span
              :style="{
                width: `${instanceInfo.maxPlayers ? Math.min(100, (instanceInfo.players / instanceInfo.maxPlayers) * 100) : 0}%`,
              }"
            />
          </div>
        </div>
        <div class="command-stat">
          ...
          <strong>{{
            hasMetrics ? `${Math.round(instanceInfo.cpu || 0)}%` : "-"
          }}</strong>
          <el-progress
        ...
        <div class="command-stat">
          ...
          <strong>{{
            hasMetrics ? `${Math.round(instanceInfo.memory || 0)}%` : "-"
          }}</strong>
          <small class="stat-note">{{
            hasMetrics ? instanceInfo.memoryUsageText || "动态采样" : "等待遥测"
          }}</small>
          <el-progress
            class="command-progress"
        ...
        <div class="command-stat">
          ...
          <strong>{{
            instanceInfo.status === "running" && hasMetrics
              ? formatUptime(instanceInfo.uptime)
              : "-"
          }}</strong>
          <small class="stat-note">{{
            instanceInfo.status === "running" ? "动态采样" : "实例未运行"
          }}</small>
        </div>
      ...
      <div class="instance-signal-band" aria-label="实例运行上下文">
        <div class="signal-cell">
          ...
          <strong class="mono-text">{{
            runtimeMetadata?.containerId || instanceInfo.id || "-"
          }}</strong>
        </div>
        <div class="signal-cell">
          <span>HOST / PORT</span>
          <strong
            >{{ instanceInfo.hostName }} · {{ instanceInfo.ip }}:{{
              instanceInfo.port
            }}</strong
          >
        </div>
        <div class="signal-cell signal-cell--wide">
          <span>WORK DIR</span>
          <strong class="mono-text">{{
            runtimeMetadata?.workDir || instanceInfo.deployPath || "-"
          }}</strong>
        </div>
        <div class="signal-cell">
          <span>CREATED</span>
    ...
    <el-card class="content-card" shadow="never">
      <el-tabs v-model="activeTab" tab-position="left" class="workbench-tabs">
          ...
          <div class="tab-content overview-workbench">
            <div class="tab-spotlight tab-spotlight--overview">
              <div>
                ...
                <h3>实例运行概览</h3>
                <p>
                  从静态配置、实时资源到最近事件，快速判断当前实例是否需要处置。
                </p>
              </div>
              <span class="tab-spotlight__signal"
                    ...
                    <div class="metrics-item">
                      <template v-if="instanceInfo.status === 'running'">
                        ...
                        <template v-else>
                          ...
                          >
                            {{
                              instanceInfo.memoryUsageText ||
                              (hasMetrics
                                ? instanceInfo.memory.toFixed(2) + "%"
                                : "-")
                            }}
                          </span>
                        </template>
                  ...
                  <el-descriptions :column="2" border>
                    <el-descriptions-item label="容器ID">
                      <span class="mono-text">{{
                        runtimeMetadata.containerId || "-"
                      }}</span>
                    </el-descriptions-item>
                    <el-descriptions-item label="Compose 项目名">
                      <span class="mono-text">{{
                        runtimeMetadata.projectName || "-"
                      }}</span>
                    </el-descriptions-item>
                    <el-descriptions-item label="工作目录" :span="2">
                      <span class="mono-text">{{
                        runtimeMetadata.workDir || "-"
                      }}</span>
                    </el-descriptions-item>
                    <el-descriptions-item
                ...
                <div class="panel-heading">
                  ...
                  <span class="refresh-state">
                    <i
                      class="status-dot"
                      :class="{
                        'is-online': instanceInfo.status === 'running',
                      }"
                    />
                ...
                <div class="resource-list">
                  ...
                  <div class="resource-row">
                    <div class="resource-label">
                      ...
                      ><strong>{{
                        hasMetrics
                          ? instanceInfo.memoryUsageText ||
                            `${Math.round(instanceInfo.memory || 0)}%`
                          : "-"
                      }}</strong>
                    </div>
                    <el-progress
                  ...
                  <div class="resource-row uptime-row">
                    ...
                    <div class="uptime-track">
                      <span
                        v-if="instanceInfo.status === 'running'"
                        class="uptime-pulse"
                      />
                    </div>
            ...
            <div class="overview-lower-grid">
              <section class="overview-panel players-panel">
                <div class="panel-heading">
                  <div>
                    ...
                    <h3>
                      在线玩家 <em>({{ instanceInfo.players || 0 }})</em>
                    </h3>
                  </div>
                  <span class="panel-muted"
                    >上限 {{ instanceInfo.maxPlayers || 0 }}</span
                  >
                </div>
                <div class="player-summary">
                  <div class="player-count">
                    {{ instanceInfo.players || 0
                    }}<small>/{{ instanceInfo.maxPlayers || 0 }}</small>
                  </div>
                  <div class="player-track">
                    <span
                      :style="{
                        width: `${Math.min(100, instanceInfo.maxPlayers ? (instanceInfo.players / instanceInfo.maxPlayers) * 100 : 0)}%`,
                      }"
                    />
                  </div>
                  <p>当前实例在线玩家</p>
              ...
              <section
                class="overview-panel recent-log-panel"
        ...
        <el-tab-pane name="config">
          <template #label>
            <span
              ><el-icon><Setting /></el-icon> 配置管理</span
            >
          </template>
          <div v-loading="configLoading" class="tab-content config-workbench">
            <div class="tab-spotlight tab-spotlight--config">
              <div>
                <span class="tab-spotlight__kicker"
                  >CONFIGURATION WORKBENCH</span
                >
                <h3>配置编排</h3>
                <p>在结构化表单与原始文件之间切换，保存前确认当前作用域。</p>
              </div>
              <span class="tab-spotlight__meta"
                ><el-icon><Document /></el-icon
                >{{ configFileName || "等待选择配置文件" }}</span
              >
            </div>
        ...
        <el-tab-pane name="files">
          <template #label>
            <span
              ><el-icon><Folder /></el-icon> 文件管理</span
            >
          </template>
            ...
            <div class="file-header">
              <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item
                  ...
                  :key="index"
                >
                  <el-button link @click="navigateToPath(index)">{{
                    part
                  }}</el-button>
                </el-breadcrumb-item>
              </el-breadcrumb>
              ...
              <el-table-column label="操作" width="200">
                <template #default="{ row }">
                  ...
                  <el-button
                    ...
                    link
                    size="small"
                    @click.stop="handleFileDelete(row)"
                  >
        ...
        <el-tab-pane name="logs">
          <template #label>
            <span
              ><el-icon><Tickets /></el-icon> 日志查看</span
            >
          </template>
          <div class="tab-content log-workbench">
            <div class="tab-spotlight tab-spotlight--logs">
              <div>
                ...
                <h3>日志流</h3>
                <p>
                  把筛选、刷新与事件正文放进同一条阅读路径，优先定位异常与上下文。
                </p>
              </div>
              <span class="tab-spotlight__meta"
                ><i
                  class="status-dot"
                  :class="{ 'is-online': logsAutoRefresh }"
                />{{ logsAutoRefresh ? "LIVE STREAM" : "PAUSED" }}</span
              >
            </div>
            <div class="log-header">
              <div class="log-filters">
                ...
                >
                  <template #suffix>
                    <el-icon class="search-icon" @click="handleLogsSearch"
                      ><Search
                    /></el-icon>
                  </template>
                </el-input>
        ...
        <el-tab-pane name="console">
          <template #label>
            <span
              ><el-icon><Monitor /></el-icon> 控制台</span
            >
          </template>
          <div class="tab-content console-tab console-workbench">
            <div class="tab-spotlight tab-spotlight--console">
              ...
              <span class="tab-spotlight__meta"
                ><i
                  class="status-dot"
                  :class="{ 'is-online': consoleConnected }"
                />{{
                  consoleConnected
                    ? "CONNECTED"
                    : consoleConnecting
            ...
            <div class="console-toolbar">
              <el-tag
                :type="consoleConnected ? 'success' : 'danger'"
                size="small"
                effect="dark"
              >
                <el-icon v-if="consoleConnecting" class="is-loading"
                  ><Loading
                /></el-icon>
                <el-icon v-else
                  ><component
                    :is="consoleConnected ? 'CircleCheck' : 'CircleClose'"
                /></el-icon>
                {{
                  consoleConnected
                    ? "已连接"
            ...
            <div ref="consoleContainerRef" class="console-output">
              <div
                v-for="(item, index) in consoleHistory"
                :key="index"
                class="console-line"
                :class="item.type"
              >
        ...
        <el-tab-pane name="plugin">
          <template #label>
            <span
              ><el-icon><Connection /></el-icon> 插件扩展</span
            >
          </template>
          <div class="tab-content plugin-tab-content plugin-workbench">
            <div class="tab-spotlight tab-spotlight--plugin">
              <div>
                ...
                <h3>游戏扩展工作区</h3>
                <p>
                  插件能力在实例上下文中运行，主应用负责承载边界与运行时身份。
                </p>
              </div>
              <span class="tab-spotlight__meta"
                ><el-icon><Connection /></el-icon
                >{{ instanceInfo.gameCode || "等待游戏识别" }}</span
              >
            </div>
            <PluginTab
...
.live-dot.is-live {
  ...
  box-shadow:
    0 0 0 4px rgba(48, 207, 116, 0.11),
    0 0 13px rgba(48, 207, 116, 0.65);
}
...
.command-header .instance-meta .el-icon {
  color: #6faec1;
}
...
.command-progress :deep(.el-progress-bar__outer) {
  background: rgba(2, 15, 25, 0.78);
}
.command-progress :deep(.el-progress-bar__inner) {
  background: #42bfe9;
}
.command-progress
  :deep(.el-progress-bar__inner.el-progress-bar__inner--success) {
  background: #48d18d;
}
...
.workbench-tabs {
  ...
  :deep(.el-tabs__item:hover) {
    color: #d3f6fd !important;
    background: rgba(22, 82, 104, 0.26);
  }
...
.uptime-track {
  ...
  .uptime-pulse {
    ...
    border-radius: 50%;
    box-shadow:
      0 0 0 3px rgba(82, 207, 130, 0.14),
      0 0 12px rgba(82, 207, 130, 0.58);
...
.recent-log-line {
  ...
  &.error .recent-log-message {
    color: var(--platform-red);
  }
  &.warning .recent-log-message {
    color: var(--platform-amber);
  }
}
...
.tab-spotlight--overview {
  border-left-color: var(--platform-green);
}
.tab-spotlight--overview .tab-spotlight__kicker {
  color: var(--platform-green);
}
.tab-spotlight--config {
  border-left-color: #b68cff;
}
.tab-spotlight--config .tab-spotlight__kicker {
  color: #c5a7ff;
}
.tab-spotlight--files {
  border-left-color: var(--platform-amber);
}
.tab-spotlight--files .tab-spotlight__kicker {
  color: var(--platform-amber);
}
.tab-spotlight--logs {
  border-left-color: var(--platform-cyan);
}
.tab-spotlight--console {
  border-left-color: var(--platform-green);
}
.tab-spotlight--console .tab-spotlight__kicker {
  color: var(--platform-green);
}
.tab-spotlight--backup {
  border-left-color: #c792ff;
}
.tab-spotlight--backup .tab-spotlight__kicker {
  color: #d3adff;
}
.tab-spotlight--plugin {
...
@media screen and (max-width: 768px) {
  ...
  .signal-cell--wide {
    grid-column: 1 / -1;
    border-top: 1px solid rgba(91, 135, 154, 0.15);
  }
  ...
  .command-action-rail {
    grid-column: 1 / -1;
    border-top: 1px solid rgba(91, 135, 154, 0.16);
  }
