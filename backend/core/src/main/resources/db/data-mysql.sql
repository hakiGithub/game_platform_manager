...
-- 2. INSERT IGNORE 防重：即使重复执行也不会因 UNIQUE 冲突中断启动。
-- 3. 默认管理员账号: admin / admin123（SHA256 加密后存储）
-- =====================================================

-- 1. 初始化管理员用户
INSERT IGNORE INTO sys_user (username, password_hash, jwt_secret, remark)
VALUES ('admin', '240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9', NULL, '系统管理员');

-- 2. 初始化游戏元数据
INSERT IGNORE INTO game_metadata (game_name, game_code, description, supported_deploy_types, default_port, environment_deps, deploy_config, custom_operations, remark)
VALUES
('Minecraft', 'minecraft',
...
-- 3. 初始化插件信息
INSERT IGNORE INTO plugin_info (plugin_id, plugin_name, version, status, description, extension_points, config_schema, author, remark)
VALUES
