...
> 关联 ADR: [ADR-0001 插件菜单归属与 getMenus() 扩展点](../design/adr/0001-plugin-menu-ownership.md)、[ADR-0006 补丁安装决策树](../design/adr/0006-patch-install-decision-tree.md)、[ADR-0007 插件前端 Night Operations token 隔离](../design/adr/0007-plugin-frontend-nightops-token-isolation.md)、[ADR-0009 平台侧能力需求](../design/adr/0009-platform-capability-requirements.md)、[ADR-0011 定时任务管理](../design/adr/0011-scheduled-task-management.md)、[ADR-0016 RCON 能力上提](../design/adr/0016-rcon-host-capability.md)、[ADR-0017 实例信息 Provider 扩展点](../design/adr/0017-instance-info-provider.md)

## 1. 版本与维护约定

| 项 | 规则 |
|---|---|
| 现行版本 | 本文档目录始终为最新版，顶部维护版本号与"对齐主应用版本/commit"标注 |
...
- v3.0.0 / v3.1.0 / v3.2.0 / v3.3.0：本文档目录（拆分整合后的分主题文件）

## 3. Changelog

| 版本 | 日期 | 变更 |
|---|---|---|
| 3.11.1 | 2026-09-13 | 插件配置（SourceModCfgService）三项修复（已部署 8081 验证）：① 候选 cfg 路径优先取插件 meta（plugin.yaml）声明的 `config_files`（中文显示名插件的 cfg 文件名取自 .smx 文件名，按显示名推导永远找不到），每条声明同时给"游戏目录副本 + 插件库副本（plugins_store/<名>/left4dead2/）"两个候选，未启用插件可从库副本查看/编辑；② SourceModCfgParser KV 正则支持不带引号的键（SourceMod 自动生成 cfg 格式），并解析 KV 行上方注释块中的 Default/Min/Max/描述；serialize 保留原行键引号风格与注释块（修复保存后行损坏）。③ `GET /plugin-config/get` 对无候选 cfg 的插件返回空配置结构（items 空数组）而非 null（前端不再误报加载失败）。配套：PluginMetaService 接入 SourceModCfgService；新增回归测试（候选 meta 优先/保存键风格保留/控制器空结构）；前端 PluginConfig 增加"恢复默认配置"按钮（调 restore-defaults，确认弹窗显示可恢复项数）与字段生效条件提示（sv_maxplayers 需 SourceMod 或 -maxplayers） |
| 3.11.0 | 2026-09-07 | 插件 API 面三项扩展（均已实施）：① `RconService` 宿主服务（[ADR-0016](../design/adr/0016-rcon-host-capability.md)）：RCON 传输层上提为主应用宿主能力，SDK 暴露 `RconService`（executeCommand / testConnection），插件按 instanceId 执行命令；端点解析只认标准键 `configInfo.rconPort`（缺省 27015）/`rconPassword`，密码不可由插件指定，每次执行自动写调用方审计日志；命令语义归插件，插件不应再自建 RCON 连接。② `InstanceInfoProvider` 扩展点（[ADR-0017](../design/adr/0017-instance-info-provider.md)）：插件以 `@Component` 实现按实例提供 `InstanceDynamicInfo`（playerCount / maxPlayerCount + extras 透传）；未实现自动降级；TTL 缓存与查询预算由主应用负责。③ `InstanceFileService` / `FileAccessService` 上传/下载新增 `FileTransferProgressCallback` 进度回调（源兼容，旧签名保留为 default 方法）。详见 `reference/host-services.md` §9-10、`reference/extension-and-menus.md` §10 |
| 3.10.0 | 2026-09-07 | 插件 API 面三项扩展（均已实施）：① `RconService` 宿主服务（[ADR-0016](../design/adr/0016-rcon-host-capability.md)）：RCON 传输层上提为主应用宿主能力，SDK 暴露 `RconService`（executeCommand / testConnection），插件按 instanceId 执行命令；端点解析只认标准键 `configInfo.rconPort`（缺省 27015）/`rconPassword`，密码不可由插件指定，每次执行自动写调用方审计日志；命令语义归插件，插件不应再自建 RCON 连接。② `InstanceInfoProvider` 扩展点（[ADR-0017](../design/adr/0017-instance-info-provider.md)）：插件以 `@Component` 实现按实例提供 `InstanceDynamicInfo`（playerCount / maxPlayerCount + extras 透传）；未实现自动降级；TTL 缓存与查询预算由主应用负责。③ `InstanceFileService` / `FileAccessService` 上传/下载新增 `FileTransferProgressCallback` 进度回调（源兼容，旧签名保留为 default 方法）。详见 `reference/host-services.md` §9-10、`reference/extension-and-menus.md` §10 |
| 3.9.1 | 2026-08-29 | 删除 skill 内 `examples/`（plugin-mygame 示例已过时）；清除指向平台仓库的相对路径链接（仅 skill 副本，docs 不受影响）。插件 API 面无变更 |
| 3.9.0 | 2026-08-29 | 文档重组（ADR-0014）：SKILL.md 瘦身为路由器，密集事实下沉分主题文件（docs 与 skill 两副本文件名对齐）；changelog 增"破坏性变更速查表"；sdk-reference 按"指路而非复制"修剪。插件 API 面无变更 |
| 3.8.0 | 2026-08-22 | ADR-0011 定时任务体系（独立于任务中心，已实施并提交）：插件新增三个接入点——`ScheduledTaskHandler`（`@Component` 一个 Handler 一个 key）、`ScheduledTaskDeclarationExtension`（声明式默认计划，宿主按 pluginId:key upsert）、`ScheduleService`（编程式 CRUD 服务，注入子容器自动绑定本插件 source，强制来源隔离）。关键语义：无自动重试（下一轮 cron 即天然重试）、不互斥、创建时不校验 handlerKey 触发才解析（未注册记 FAILED）、同一计划同时刻只允许一个 run（重叠记 SKIPPED）、停机不补跑、payload 快照传 Handler、run 日志 500 条上限 + 30 天清理。插件生命周期联动：停用/热重载（purgeTasks=false）暂停、重载恢复、卸载移除（purgeTasks=true）物理清理。主界面「任务中心 → 定时计划」统一管理。新增陷阱：声明式计划被用户改过（userModified）会被 upsert 跳过、删过不复活；热部署勿用 purgeTasks=true（会删计划）。详见 `reference/scheduled-tasks.md` 与 [ADR-0011](../design/adr/0011-scheduled-task-management.md) |
