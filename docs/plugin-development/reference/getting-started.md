...
| 项 | 规则 |
|---|---|
| 现行版本 | 本文档目录始终为最新版，`CHANGELOG.md` 维护版本号与"对齐主应用版本/commit"标注 |
| 物理快照 | 每逢**主版本**变更（插件 API 破坏性改动或重大重构），将旧版另存归档保留 |
| Changelog | `CHANGELOG.md` 维护；主应用新增/变更 API 时追加条目并升版本号 |
...
打包为单个 JAR 放入主应用 `plugins/` 目录，启动主应用即被加载。验收标准见 `reference/checklist.md`。
