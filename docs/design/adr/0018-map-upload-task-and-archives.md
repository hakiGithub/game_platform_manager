...
### 决策 5bis：自动裁剪防呆（2026-08-30 事故教训）

实测事故：上传 workshop 脚本类插件（`1602953807.vpk`，83.7MB 客户端资源包）后，自动裁剪把
其削成 1.1KB 残桩（仅存 vscripts），srcds 加载该残桩即段错误，容器陷入崩溃循环（二分实证：
移除残桩即恢复，放回即复现）。防呆规则：**裁剪产物不足原文件 1% 时视为客户端资源型 VPK，
放弃覆盖、保留远端原文件**（`trimMap` 内置，不影响正常战役地图的常规裁剪比例）。同批事故
的另一独立诱因是容器运行时 `$HOME/.steam/sdk32/steamclient.so` 缺失（镜像层问题，已在容器内
修复；容器重建会丢失，持久化需进镜像或 compose command）。
