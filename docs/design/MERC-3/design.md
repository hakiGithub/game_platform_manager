...
| **创建时间** | 2026-09-20 |
| **版本** | v0.3.5 |
| **状态** | 第 2 轮复审 **PASS 已接受（条件解除）**；v0.3.2 / v0.3.3 两层门禁齐、无前置条件带入 G1；v0.3.4 = G1 零轮次记账批。**本版 v0.3.5 = SUG-21 的裁定落文批**（Leader 裁定评论 `01a0c4d8`：**采用拆分**，不选单支、不挂实现期）：§14.6 `stage` 行给 `stepId == null` 的两类条件行分别钉值（**BR-12 拦截行 = `"EXTENSION"`**；**目录不合法说明行 = 其实际发生所在的既有阶段，本期 `"DEPLOY"`**），§10 V-08 首判据的取值域写明为「**扩展阶段内的行**」、不再读作 logs 全部行，并**按同一裁定的书面授权**把 §3.2 `:81` 的 EXTENSION 段存在条件与「入口判定即终止」一支对齐（同一缺陷，拆开＝各修一半）。**改动面锁死在这三处**：§14.6 `stepId` 行、§14.6 规则 2、§16.4 `INVALID` 行、§14.11 latch 行、§3.2 `:82` 段内序列**逐字未动**（核对见 §17 v0.3.5 自查表）；依赖「EXTENSION 段存在条件」的相邻五行（§7.2 B-10 `:230`、§7.3 F-02 `:248`、§10 V-10 `:371`、§10 V-12 `:373`、§14.7 `:712`）逐行核对并**分两类登记**：三行判据对象是「无扩展声明的游戏」⇒ 与本裁定不矛盾、未动；**B-10 与 V-12 两处的字面（「无步骤路径」/「无扩展步骤：整条序列不含 `EXTENSION`」）在本裁定下能罩到 BR-12 那一路**——两处的判据对象仍是 AC-15 那批游戏，要判的东西不变，但留了一处可被 @Tester 读宽的缝 ⇒ **属超授权面，一字未改，连最小改法一并报 Leader**（`01a0c4d8` 第 2 条：超出「EXTENSION 段的存在条件」这一句即停下先报）。KPI-02 的分母口径与「`stepId == null` 不进分母」一字未动。零轮次记账延续、不占评审轮次。待 Leader 通用门禁复跑与 G1 汇总（本裁定在 G1 单列一节供人类一票否决） |
| **Issue** | MERC-3 |
...
| **相邻面核对：两行字面被本裁定读宽** | **超授权面 ⇒ 未改，报 Leader 定** | §7.2 B-10 `:230`、§10 V-12 `:373` | 两处的判据对象仍是 AC-15 那批无扩展声明的游戏、要判的东西一字不变，但**字面**用的是「无步骤路径的进度值与阶段序列逐值不变」/「**无扩展步骤**：整条序列不含 `EXTENSION`」——BR-12 那一路零步骤而序列含两条 `EXTENSION` 行，字面上会被罩进去，是 @Tester 落脚本时真会踩的一格。**最小改法（各一处，不在本版自行落文）**：V-12 该短语改「**无扩展声明（未触发 BR-12）**」；B-10 的「无步骤路径」改「**无扩展声明游戏的路径**」。Leader 若判「不补」，本表这两行即为该判断的记录处；若判补，属另一次自然触手，不占本版 |
