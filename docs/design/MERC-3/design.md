...
| **创建时间** | 2026-09-20 |
| **版本** | v0.3.12 |
| **状态** | 第 2 轮复审 **PASS 已接受（条件解除）**；v0.3.2 / v0.3.3 两层门禁齐、无前置条件带入 G1；v0.3.4 = G1 零轮次记账批；v0.3.5 = SUG-21 的裁定落文批（Leader 裁定 `01a0c4d8`：§14.6 `stage` 行两行分别钉 / §10 V-08 取值域收敛为「扩展阶段内的行」/ §3.2 EXTENSION 段存在条件两支化）。**v0.3.6 = v0.3.5 报上去的两处「超授权面」字面的授权落文批 + 元数据锚点统一批**（Leader 裁定评论 `01a0c512` 第 ① ② 条，合一版单次提交）：**①** §7.2 B-10 的「无步骤路径」改「**无扩展声明游戏的路径**」、§10 V-12 的「无扩展步骤」改「**无扩展声明（未触发 BR-12）**」——两处只把字面的判据对象从「无步骤 / 无扩展步骤」收紧为「无扩展声明」，使 v0.3.5 裁定下 BR-12 那一路（零步骤而序列含两条 `EXTENSION` 行）不再被读宽；**判据对象（仍是 AC-15 那批无扩展声明的游戏）与要判的东西一字不变，核对物不增不减**；**②** 元数据「上游 PRD」锚点自 `270f9d0`（v0.5）回改至 `ee86825`（v0.6 回写批），并在该格把锚点语义钉死为「**对读基准版本**」——metadata-only 的升版不触发反向回改，故 PRD 侧不必回锚本版，两文件不互相追版本（v0.3.5 登记的那处「已知滞后」在此闭合）。**本版判定文本改动恰这三行**；v0.3.5 未授权锚点清单里的其余 **20 处**（§3.2 段内序列行与终止条件行、§16.4 `INVALID`/`EMPTY` 两行、§14.11 小节标题与 P3 latch 行、§14.6 `stepId`/规则 2/`stepEvent`/`elapsedMs` 四处、§7.3 F-02、§10 V-10、§14.7 无扩展步骤行、§14.13.2 落点行、元数据「上游 ADR」「平行输入」两行、§18 的 v0.2 / v0.3.3 / v0.3.4 / v0.3.5 四条历史行）**逐字未动**，那三行则在本版核对里**转为授权改动并逐字断言新文本**。v0.3.5 刚钉下的三处（§14.6 `stage` 行 / §10 V-08 首判据 / §3.2 存在条件）另组核对、亦逐字未动。KPI-02 的分子分母口径一字未动。零轮次记账延续、不占评审轮次。待 Leader 通用门禁复跑与 G1 汇总（该批随后已随 G1 汇总交人类并通过评论 `01a0c540`；本格按「历史句不改写、指针只在新增处给」的既有口径保留 v0.3.6 原句）；**本版 v0.3.7 = 交付分支名统一批（metadata-only）**（Leader 派工评论 `01a0c552`）：元数据「交付分支」格与 §9.1「分支与 ADR 载体」行仍写 MERC-3 时期拟定的旧分支名，与 Leader 在 G1 汇总 `01a0c526` 声明的 `release/MERC-8-extension-steps-dnf-tw` 不一致 ⇒ **全部出现处（实测恰 2 处）一次 grep 收净、本版后全文旧名命中 0**；两行都由整行等式证明是**纯值替换**，§9.1 该行携带的另一件事（ADR 与 `CONTEXT.md` / `glossary.md` 须从 `agent/leader/chat-1cfa252af963 @ 590af8d` merge 带入）一字未动 ⇒ 授权边界里「凡需改语义处停下先报」的**停止条件未触发**。**本版判定文本零改动**：被改写的既有行恰 5 行、全是元数据与版本链记账 + §9.1 的一处字面值，`790ccd6` 其余 1258 行逐字保留（整文件多重集差全量证明、非抽样）。metadata-only 升版按 v0.3.6 钉死的「对读基准版本」语义不触发反向回改 ⇒ `prd.md` 与 `docs/ui/MERC-3/*` 零改动。KPI-02 的分子分母口径、§14.6 `stage` 行、§10 V-08 首判据、§3.2 存在条件、B-10 / V-12 两行**逐字未动**。零轮次记账延续、不占评审轮次。待 Leader 通用门禁复跑。**v0.3.8 追加的现行状态（只做加法，上方各版旧句一字未删）**：G1 已由人类通过（评论 `01a0c540`「通过，按推荐」）⇒ 本文自此是**实现与验收的基线文本**；v0.3.7（交付分支名统一）与本版 v0.3.8（§6.3 UI 锚点同源）都是 G1 收口之后的**文档同步批**，不改判定、不新增事实。上方两处「待 Leader 通用门禁复跑」措辞（v0.3.6 与 v0.3.7 各自的收尾处）按「历史句不改写」口径原样保留，其现行读法以本句为准——都已是过去时。**v0.3.9 追加的现行状态（只做加法，上方各版旧句一字未删）**：本版仍是收口后的文档同步批——§3.3 组 D 与 §14.10 方案 B 两处三参签名对齐到已交付实现（取 2 参、实现不动、改文档，Leader 裁定评论 `01a0cf20` §四），另在 §16.3 的 N 清单旁加一枚「撞键校验只在提交期」的防回潮指针；**判定正文被改写的既有行恰那两行签名、其余全为纯插入**。`prd.md` 侧的对应收敛是同批的 v0.7，本版两处改动不依赖 PRD 的任何新增文本，故按 v0.3.6 在「上游 PRD」格钉死的「对读基准版本」语义**不回改该格、也不新设第二枚基准**。 **v0.3.10 追加的现行状态（只做加法，上方各版旧句一字未删）**：本版仍是收口后的文档同步批，形制与 v0.3.9 同类——三处按已交付代码的机械更正（Leader 处置评论 `01a0d42d` §三-A，触发点是 MERC-20 的交付回传 `01a0cf5e` §4）：§15.2「实现落点」一行的前提被代码事实推翻、§14.14 的 holder 后缀取不到那枚步骤序号、§8.4 对 `.platform-extension/` 的删除粒度写粗了。**判定正文被改写的既有行恰这三行，另有两行是在原段内加限定的纯插入；不新开判据、不新增编号、判定对象与核对物一字不变**。`prd.md` 与 `docs/ui/MERC-3/*` 按「对读基准版本」语义不回改。 **v0.3.11 追加的现行状态（只做加法，上方各版旧句一字未删）**：本版仍是收口后的文档同步批，形制与 v0.3.10 同类、且是它的**续批**——v0.3.10 登记为「只报不改」的同类相邻滞后四处，经 Leader 重派评论 `01a0d45d` 第 1 条授权按同一纪律落文：两枚是 §15.2 那处 `timeoutMs` 旧读法在本文的另两处落点（§1 引导句、§15.1 首行），两枚是裸命令形状在本文的另两处落点（§7.2 B-13 步骤列、§8.4「脚本注入」行）。**判定正文被改写的既有行恰这四行、零新增行；判定对象、编号、核对物一字不变**（B-13 的核对物列逐字未动、§15.1 那行的取值 30 s 与出处 `FileAccessService.java:264` 逐字未动）。另登记同族表述三处仍只报不改（§15.2「缺省值」行的「宿主机通道默认」括注、§15.3 首行、§12 RISK-D06 行）。`prd.md` 与 `docs/ui/MERC-3/*` 按「对读基准版本」语义不回改。 **v0.3.12 追加的现行状态（只做加法，上方各版旧句一字未删）**：本版仍是收口后的文档同步批，形制与 v0.3.10 / v0.3.11 同类、是 v0.3.11 的**续批**——v0.3.11 登记为「只报不改」的同族表述三处，经 Leader 派单评论 `01a0d484` §五-1 ① 授权按同一纪律落文：§15.2「缺省值」行那枚把 30 s 称作通道默认值的括注、§15.3 首行、§12 RISK-D06 行，三处都只把「不再等待 / 不保证已终止」所属的机制从通道入参 `timeoutMs` 改指到命令文本里的远端 `timeout`（v0.3.10 已在 §15.2「实现落点」行钉过的同一代码事实，非新增知识）。**判定正文被改写的既有行恰这三行、零新增行；结论、取值、编号、判据、登记对象一字不变**（§15.2 缺省值仍是 `600_000 ms`、§15.3 的「不保证远端进程已被终止」与三条子弹逐字未动、RISK-D06 的编号与缓解格未动）。另按 Leader §二 裁定的口径给出引注的 main 侧逐条对账读数（只报不改，release / 交付头的漂移只登记）。`prd.md` 与 `docs/ui/MERC-3/*` 按「对读基准版本」语义不回改。 |
| **Issue** | MERC-3 |
| **上游 PRD** | docs/prd/MERC-3/prd.md @ `ee86825`（v0.6 技术设计回写批：§13.7 D-P01～D-P17 全 17 项 + SUG-21 拆分裁定）。**本行锚点的语义 = 对读基准版本**（v0.3.6 按 Leader 裁定 `01a0c512` 第 ② 条自 `270f9d0`/v0.5 回改，并在此钉死语义）：它只声明「本文这版判定是对着哪一版 PRD 读的」，**不是**「两文件版本必须同步」——两文件互为锚点属循环，故 **metadata-only 的升版不触发反向回改**：本版只把锚点追到 PRD v0.6，PRD 侧不必回锚本版 v0.3.6，此后两文件不互相追版本。（v0.3.5 在 §18 / §17 登记的那处「已知滞后」至此闭合；该行携带的交叉引用无需再复核——本文对 PRD 的引用点不落在本格。） |
...
| **交付分支** | `release/MERC-8-extension-steps-dnf-tw` |

> 本文按「先落盘再细化」分次提交：v0.1 骨架（§1/§2 + 待判定清单）→ v0.2 补齐 §3–§16 → **v0.3 并入架构评审第 1 轮（REV-1…REV-7 全采纳 + SUG-1…SUG-9）** 与 Leader 的两项范围裁定（每主机互斥取「补一层等价互斥」、EXTENSION 支持集合按 FR-11 字面收口为 compose 两类）→ **v0.3.1 并入架构评审第 2 轮新发现的九条（SUG-10…SUG-18）定点落文** → **v0.3.2 并入第 2 轮定点确认新增的两条（SUG-19 / SUG-20），改法按 Leader 裁定 `01a0bf2b` 定死** → **v0.3.3 按 Leader 裁定 `01a0bf39` 把 §10 V-08 与 §14.6 规则 2 的两处六项说明性括注各补一项成七项（同一集合的项数同步，无新判定）** → **v0.3.4 = G1 零轮次记账批**（SUG-20 表末格补已授权标记 + SUG-22 自查块排序 + 元数据「平行输入」锚点随 UI v0.6 定稿统一并复核该行携带的交叉引用；**SUG-21 经核对与 v0.3.3 既有内容相撞 ⇒ 只登记、不改判定文本，交 Leader / @Architect 裁定**）→ **v0.3.5 = SUG-21 的裁定落文批**（按 Leader 裁定 `01a0c4d8` 采用「两行分别钉」的拆分：§14.6 `stage` 钉 BR-12 拦截行 = `"EXTENSION"` / 目录不合法说明行 = 实际发生所在的既有阶段（本期 `"DEPLOY"`）；§10 V-08 首判据取值域收敛为「扩展阶段内的行」；§3.2 `:81` 存在条件与「入口判定即终止」一支同批对齐）→ **v0.3.6 = 授权补项 + 锚点统一批**（Leader 裁定 `01a0c512` 第 ① 条：v0.3.5 报上的两处超授权面字面，按其自给的最小改法落文——§7.2 B-10 / §10 V-12 各一处，判据对象与核对物不变；第 ② 条：元数据「上游 PRD」锚点回改至 PRD v0.6 `ee86825`，锚点语义钉为「对读基准版本」、metadata-only 升版不触发反向回改）。 → **v0.3.7 = 交付分支名统一批**（Leader 派工 `01a0c552`：本文元数据「交付分支」格与 §9.1「分支与 ADR 载体」行仍写 MERC-3 时期拟定的旧分支名，与本 Issue 在 G1 汇总 `01a0c526` 声明的交付分支 `release/MERC-8-extension-steps-dnf-tw` 不一致 ⇒ 全部出现处一次 grep 收净；**范围锁死为分支名替换、判定文本零改动**，metadata-only ⇒ 按 v0.3.6 钉死的「对读基准版本」语义不触发反向回改）。 → **v0.3.8 = §6.3 UI 锚点同源批**（Leader 评论 `01a0c56e` 授权 A / B 两条：A 把 §6.3 那枚仍指 UI v0.1 的旧锚点统一至 `62b49a9`、并加一枚指向元数据「平行输入」格的同源指针；B 给元数据状态格做加法、追加一句现行状态。**范围锁死为这两条、判定文本零改动**，metadata-only ⇒ 按 v0.3.6 钉死的「对读基准版本」语义不触发反向回改）。 → **v0.3.9 = `applyVersionSelection` 签名对齐 + §16.3 防回潮指针批**（Leader 裁定评论 `01a0cf20` §四：取 2 参、实现不动、改文档——§3.3 组 D 与 §14.10 方案 B 两处三参签名统一为 `applyVersionSelection(configInfo, catalog)`，第三参由载荷 `configInfo.deployVersion` 取代；§16.3 的 N 清单旁加一枚「撞键校验只在提交期、N1…N5 刻意不含撞键规则」的防回潮指针。**判定正文被改写的既有行恰那两行签名、其余全为纯插入**；单独 commit、只触本文件、禁 force-push）。 → **v0.3.10 = 三处代码事实更正批（MERC-20 交付回读）**（Leader 处置评论 `01a0d42d` §三-A：**①** §15.2「实现落点」行按代码事实更正——`executeCommand` 的 `timeoutMs` 只兜住建连与认证（`SshUtil:356` → `getOrCreateSession`），命令执行走 `executeRemoteCommand` 无超时（`SshUtil:368`）⇒ 脚本耗时的实际护栏在命令文本里：`timeout <秒> bash <file>`，秒粒度**向上**取整；**②** §14.14 的 holder 形状更正为 `EXT:<instanceId>:<进程内自增>`（那枚步骤序号在定稿的 SDK 签名里不可得）；**③** §8.4 的 `.platform-extension/` 收口粒度细化为「删自建脚本文件 + 尝试 `rmdir`，非空即保留」。**判定正文被改写的既有行恰这三行、另有两行原段内加限定的纯插入；判定对象、编号、核对物零改动**；单独 commit、只触本文件、禁 force-push）。 → **v0.3.11 = 同类相邻滞后续批（把 v0.3.10 那四处「只报不改」落文）**（Leader 重派评论 `01a0d45d` 第 1 条授权：**①** §1 引导句与 **②** §15.1 首行按 `timeoutMs` 的真实担保范围更正读法（它只兜住建连与认证，命令执行无超时）；**③** §7.2 B-13 步骤列与 **④** §8.4「脚本注入」行的命令形状补齐 `timeout <秒>` 前缀，与 §15.2（v0.3.10）和已交付实现对齐。**判定正文被改写的既有行恰这四行、零新增行；判定对象、编号、核对物零改动**；另登记同族表述三处只报不改，交 Leader 判；单独 commit、只触本文件、禁 force-push）。 → **v0.3.12 = 同族表述三处的归因更正批（把 v0.3.11 那三处「只报不改」落文）**（Leader 派单评论 `01a0d484` §五-1 ① 授权：**①** §15.2「缺省值」行 :945 那枚把 30 s 称作通道默认值的括注、**②** §15.3 首行 :956、**③** §12 RISK-D06 行 :452——三处只改归因（「不再等待 / 不保证已终止」由通道入参 `timeoutMs` 改指命令文本里的远端 `timeout`），**结论、取值、编号、判据、登记对象零改动**；另附引注 main 侧逐条对账读数（只报不改）。判定正文被改写的既有行恰这三行、零新增行；单独 commit、只触本文件、禁 force-push）。
>
...
>
> **v0.3.5 的性质**：**一条被 Leader 裁定收口的判定缺口落文**（裁定评论 `01a0c4d8`：采用拆分，不选单支、不挂实现期），不是新口径、不是翻判。缺口本体是 §14.6 `stage` 行只写了「扩展阶段全部行取常量 `"EXTENSION"`」，而七项阶段级行里的两类**条件行**（`stepId == null` 的 BR-12 拦截行 / 目录不合法说明行）分属两种归属：一支钉 `EXTENSION`、一支钉它实际发生的既有阶段。裁定依据是本设计在 v0.3.4 给出的两组 UI 实物——统一取 `EXTENSION` 被 `LG`（无带、无进入行）证伪，统一取既有阶段被 `LE`/`LF`（有带 + 入口行）证伪 ⇒ **拆分是唯一与实物、§14.11 latch、§16.4 `INVALID` 同时自洽的写法**；`ui-spec.md @ 62b49a9` 侧的两处既有定稿文字与本裁定同向——§4.2 `P`/`Q` 行给 BR-12 两支的阶段带列写「**渲染**」（且 §13.3「⑧b」行称其为「**入口拦截、零步骤**」），同一行末段把 `LB` / `LG` 归入「**无阶段带**、与本条无关的屏：未进入扩展阶段」⇒ **钉值与 UI 实物同源，本版不需要 UI 侧回改**。本版动三处：§14.6 `stage` 行钉两行取值、§10 V-08 首判据取值域收敛为「扩展阶段内的行」、§3.2 `:81` 存在条件与「入口判定即终止」一支对齐（第三处是裁定书面授权的同批对齐——同一缺陷拆开修＝各修一半）。**未动**：§14.6 `stepId` 行的七项枚举与「定死」措辞、规则 2 与 V-08 的 KPI-02 分母口径（两行 `stepId == null`，取何值都不进分母，本版的取值收敛与 SUG-13 建立的可满足性论证无关）、§16.4 `INVALID` 行、§14.11 latch 行、§3.2 `:82` 段内序列、三条硬判定结论、§14.0 / §14.0.1 十四行、`docs/ui/MERC-3/*`、功能代码。**依赖该存在条件的相邻五行逐行核对后分两类：三行判据对象是「无扩展声明的游戏」⇒ 与本裁定不矛盾；B-10 `:230` 与 V-12 `:373` 两处的字面在本裁定下能罩到 BR-12 那一路（要判的东西不变、但留了一处可读宽的缝）⇒ 两处均按超授权面处理，一字未改，连最小改法一起报 Leader**（`01a0c4d8` 第 2 条：超出「EXTENSION 段的存在条件」这一句即停下先报）。逐行结论见 §17「v0.3.5 裁定落文」表末两行。
>
> **v0.3.6 的性质**：**两笔已授权的补项，无新判定**。① 是 v0.3.5 按「超出『EXTENSION 段的存在条件』这一句即停下先报」的边界**一字未改、连最小改法一起报上去**的那两处字面 —— Leader 在裁定评论 `01a0c512` 第 ① 条判「补」（理由：不补就是把这格缝留给 G1 之后的 @Tester 脚本），并指名按本设计自给的最小改法落：§7.2 B-10「无步骤路径」→「**无扩展声明游戏的路径**」、§10 V-12「无扩展步骤」→「**无扩展声明（未触发 BR-12）**」。两处改的都是**主语的口径**而不是判据：改前要判的东西（进度值与阶段序列逐值不变 / 整条序列不含 `EXTENSION`）与改后完全同一句，核对物不增不减，判据对象仍是 AC-15 那批无扩展声明的游戏 —— 收紧的只是「零步骤」这一形态在 v0.3.5 裁定下不再等同于「无扩展声明」（BR-12 那一路零步骤却产两条 `EXTENSION` 行）。**不因此动其余三行**（§7.3 F-02 / §10 V-10 / §14.7）：v0.3.5 已核对它们的主语本就是「无扩展声明的游戏」、与本裁定不矛盾，本版复核为真。② 是 v0.3.5 登记的「元数据『上游 PRD』锚点仍指 v0.5」那处已知滞后 —— 回改至 PRD v0.6 `ee86825`，并把锚点语义钉死为「**对读基准版本**」：metadata-only 的升版不触发反向回改，PRD 侧不必回锚本版，两文件不互相追版本（不钉这句，下一次 PRD 升版又会提出同一个「要不要再回锚」的问题）。**本版判定文本恰动三行**：§7.2 B-10、§10 V-12、元数据「上游 PRD」行。v0.3.5 刚钉下的三处（§14.6 `stage` 行 / §10 V-08 首判据 / §3.2 存在条件）、KPI-02 的分子分母口径、§14.6 `stepId` 行的七项枚举与「定死」措辞、§16.4 两行、§14.11 两行、§3.2 段内序列与终止条件、§18 四条历史行、元数据「上游 ADR」「平行输入」两行**逐字未动**；`prd.md`、`docs/ui/MERC-3/*`、功能代码零改动。逐处核对见 §17「v0.3.6 授权落文」自查表。
>
> **v0.3.7 的性质**：**一处文档滞后的一次性收净——判定文本零改动、不新增任何事实**。本 Issue 的交付分支由 Leader 在 G1 汇总（评论 `01a0c526`）声明为 `release/MERC-8-extension-steps-dnf-tw`，而本文元数据「交付分支」格与 §9.1「分支与 ADR 载体」行仍写 MERC-3 时期拟定的旧名；拆票回传（评论 `01a0c552`）把它点名为「声明的分支名与实现票面唯一的不一致处」，Leader 同评论授权本批并**把范围锁死为分支名替换**：全部出现处一次 grep 收干净、凡需改语义处停下先报、单独 commit、只触本文件、禁 force-push。实测旧名出现处**恰 2 处**、正是授权点名的两格，且两处都由整行等式 `旧行.replace(旧名, 新名) == 新行` 证明是**纯值替换**——§9.1 那行携带的另一件事（`docs/design/adr/0029-*.md` 与 `CONTEXT.md` / `glossary.md` 须从 `agent/leader/chat-1cfa252af963 @ 590af8d` **merge 带入**）一字未动，新名下复核仍为真 ⇒ **停止条件未触发**，本版没有任何一处需要改语义。写法上另取一条自律：本版全部记账文本**不复现旧名字面串**（只以「MERC-3 时期拟定的旧名」指代），故本批不像 v0.3.6 那样要靠「旧字面只作订正说明、不作现行断言」来判——裸 grep 即干净。本批是 metadata-only 升版：按 v0.3.6 在「上游 PRD」格钉死的「**对读基准版本**」语义，它不触发任何反向回改，`prd.md` 与 `docs/ui/MERC-3/*` 因此零改动；文档与 ADR 的目录名 `docs/*/MERC-3/` 按 MERC-8 的继承约定**不重命名**（避免与已 push 产物的路径错位），故本版之后「目录写 MERC-3、交付分支写 MERC-8」这一形态仍然成立，只是**分支名自此无处是旧的**。另登记一处相邻滞后**未改**（不是分支名、且改它需连带判一句 UI 定稿状态的现行真假 ⇒ 超出授权面）：§6.3 的「UI 词面归 ui-spec §6.2」那句仍指 `f6312ef`（UI v0.1），与元数据「平行输入」格的 `62b49a9`（v0.6）不同源。逐处核对见 §17「v0.3.7 交付分支名统一」自查表。
>
> **v0.3.8 的性质**：**一处「同一文件内两枚 UI 锚点不同源」的收净——判定文本零改动、不新增任何事实**。v0.3.7 扫 UI 锚点时暴露并登记：元数据「平行输入」格已在 G1-b 统一至 `62b49a9`，而 §6.3 那句「UI 词面归 ui-spec §6.2」仍带一枚指向 UI v0.1 的旧锚点、并挂着「S1b v0.4 在途」这句状态短语；v0.3.7 按「改它须连带判一句 UI 定稿状态的现行真假」的授权边界**停下先报、只登记未改**（§17 该行）。本版之所以能改，是 Leader 在评论 `01a0c56e` 补齐了当时缺的那条依据，两条都在本文一侧可核：**其一**，元数据「平行输入」格自身已把 `62b49a9` 读作「v0.6 / S1b 定稿」（v0.3.4 的 G1-b 复核落文），本文并不缺少判断标准，缺的是允许触格的授权；**其二**，G1 已由人类通过（评论 `01a0c540`「通过，按推荐」）⇒ 「S1b v0.4 在途」自此不再是可以留在正文里的现行断言，它与本文自己的元数据互相打脸。落文两条，范围锁死：**A** §6.3 那枚锚点取**一个连续子串**的统一——旧值换成 `62b49a9`、状态短语换成「v0.6 / S1b 定稿」、同一处加一枚**指向元数据「平行输入」格的并收状态指针**；于是那句「UI 侧并收仍挂账」的 caveat 全文只留那一格，正文不复制第二份。**换锚点前先按 G1-b 立的规矩复核该行携带的交叉引用在新锚点下不落空**：`ui-spec.md @ 62b49a9` 的 §6 章抬头即「文案矩阵（词面定稿）」、§6.2「阶段与步骤行」在位 ⇒「UI 词面归 ui-spec §6.2」在新锚点下仍为真，这与 G1-b 那次「只换 hash 会把一条正确的断言静默变成错的」是同一件事的正向做法。**B** 元数据状态格**只做加法**：追加一句现行状态（G1 已通过、本版为实现基线；v0.3.7 / v0.3.8 为收口后的文档同步批），上方 v0.3.5 / v0.3.6 / v0.3.7 各句一字未删，两处过期的「待 Leader 通用门禁复跑」措辞按「历史句不改写 + 现行读法以新句为准」处理。**停止条件未触发**：A 只动 §6.3 那一个连续子串（由整行等式证明，不是词级 diff 的近似），该行携带的判据集合——六个字段位、`stepEvent` 取值、五条归组与判定规则、`elapsedMs` 单位、`exitCode` 承载位、`stdout` 与 `stderr` 的非判据地位、`stage = "EXTENSION"` 常量、契约与词面「判据 / 渲染」的关系——一字未动，未连带改到别的现行断言。**判定文本零改动**由整文件多重集差给出**全量证明**：被改写的既有行恰 5 行（元数据「版本」与「状态」两格、版本链一句、§6.3 那行、§17 那行 v0.3.7 表末行的授权指针），落在 §1–§16 判定正文的恰 §6.3 一行，而它改的是锚点字面值而非判据 ⇒ v0.3.5 钉的三处、v0.3.6 落文的三行、v0.3.7 统一的两格、元数据「平行输入」格自身、KPI-02 的分子分母口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、§18 历史行**零回退**是被覆盖的结论，不依赖抽样选得准不准。另立一条**类别收净**，不止修点名的那一处：全量扫本版 §1–§16 的 commit 形态字面，除本行那枚旧锚点外**无第二枚陈旧锚点**（其余命中为 ADR 的 `590af8d` 与 UI 的 `62b49a9` 两枚现行值、评论号、以及 `1800000` / `1200000` 这类数字假阳性）⇒ 本文对 UI 的两枚锚点自此同源，这一类滞后无处再留。写法上沿用 v0.3.7 那条已获认可的自律：**本版全部新写的记账文本不复现旧锚点字面串**（只以「UI v0.1 那枚旧锚点」指代），故全文剩余的旧锚点命中全部是 v0.3.4 / v0.3.7 既有历史行、本版新增 0 处。本批是 metadata-only 升版：按 v0.3.6 在「上游 PRD」格钉死的「**对读基准版本**」语义不触发任何反向回改 ⇒ `prd.md`、`docs/ui/MERC-3/*`、功能代码零改动。逐处核对见 §17「v0.3.8 §6.3 UI 锚点同源」自查表。
>
> **v0.3.9 的性质**：**一处签名与已交付实现对齐 + 一枚防回潮指针——判定正文除那两行签名外零改动、不新增任何事实**。触发点是 MERC-18 的交付回传（评论 `01a0cf18` §二）把它列为已知限制：§14.10 方案 B 的表格里写着三参形态的签名，比实现多一个入参。Leader 在评论 `01a0cf20` §四 裁定「**取 2 参、实现不动、改文档**」，依据三条：① §6.1 的 `POST` 格已写「无新增字段：版本以 `configInfo.deployVersion` 承载」；② §16.4 末段已把「写 / 删 / 不写」的决定钉在载荷上；③ 独立的选择入参与载荷同源 ⇒ 同一事实两份真相，谁先生效无从判定；那枚入参名自此在本文全文消失。§14.10 那处签名在**方案对比表内**（A / B 两案的取舍叙述），不是签名契约，故改它不触及任何判据；两行都由**整行等式** `旧行.replace(旧子串, 新子串) == 新行` 证明差**各恰一段**。第二件事是 §16.3 的 N 清单旁一枚**防回潮指针**：撞键校验只在提交期、N1…N5 刻意不含「`deployVersion` 与 `variables[].name` 同名」这一条；它的对立面（补第 6 条声明期规则）由 `prd.md` 同批的 v0.7 关掉，两处同源、互为指针。**本版不新增任何事实**：不引入真实版号 / URL / 目标路径，不新增设计编号；按 v0.3.6 钉死的「对读基准版本」语义不替 PRD 回改锚点、也不在判定正文新设第二枚 PRD 锚点（v0.7 的 commit 只出现在记账区）。逐处核对见 §17「v0.3.9 签名对齐」自查表。
>
> **v0.3.10 的性质**：**三处「本文的断言与已交付代码对不上」的机械更正——判定对象与核对物零改动、不新增任何事实**。触发点是 MERC-20 的交付回传（评论 `01a0cf5e` §4）与 Leader 的处置（评论 `01a0d42d` §一 ②③⑤、§三-A）；三处同一形态：本文写作时依据的是既有代码的形状，而能力面落地时代码事实与本文不同。① **§15.2「实现落点」行**原写作该通道的入参足以兜住整条命令的执行——实际是 `SshUtil` 的 `timeoutMs` 只作用于建连与认证（`:356` → `getOrCreateSession`），命令执行走 `executeRemoteCommand` 不带超时（`:368`），挂死的脚本能把部署线程永久阻塞。同一事实本文另一处早已写明：§14.13.3「命令」行为 compose 类兜 `timeout 1200` 时给的就是这个理由，两适配器 `DEPLOY` 的现场注释亦同（`DockerComposeAdapter:259-261`、`LinuxGsmDockerAdapter:222-223`，两处都逐字写着「`SshUtil` 的 `timeoutMs` 仅作用于建连」）⇒ **本行是同一事实的第二处落点、v0.3 写作时漏改，这次更正把孤立的一行拉回本文自己的口径，不是新增知识**。护栏自此落在命令文本上：`timeout <秒> bash <file>`，秒粒度**向上**取整——向下取整会在声明的预算用满前就把脚本杀掉，属 BR-04 禁止的「主应用推断或覆盖声明」。通道本身（`SshUtil` / `FileAccessService`）一字未动 ⇒ 「本期零通道改造」的结论仍成立，只是它担保的不是这格。② **§14.14 的 holder** 伪代码把步骤序号当后缀，而 `installSync` 的定稿 SDK 签名是两参、拿不到序号（序号只作为 §14.6 的 `LogEntry` 字段位存在于呈现层）；实测后缀取进程内自增，而**「后缀两路互不相同」是正确性条件不是风格**：`TaskMutexManager.putIfAbsent` 的防御支 `return existing.equals(taskId)`（`:52`）会让共用同一 holder 的两路都判「占用成功」，其 `finally` 的 `remove` 又是按值 CAS 删除（`:68`）⇒ 后进入的一路会把先进入那一路正持的键释放掉。前缀一字未动 ⇒ 本节「与任务中心的边界」行与 §10 V-26 的 holder 判据（只认前缀）取值与判定均不变。③ **§8.4 的 `.platform-extension/`** 一句原写作无差别的删净，实测粒度是「删平台自建的脚本文件 + 尝试 `rmdir` 该目录、非空即失败并原样保留」；用 `rmdir` 而非 `rm -rf` 是刻意的——脚本自己往该目录写过的产物不由平台顺手清空。**登记同类相邻表述四处未改（两类，超出本次点名的三格，按「先报后判」交 Leader，本版只报不改）**：§1 引导句 `:47` 与 §15.1 首行的「默认 30s / 宿主机命令通道默认超时」仍是 `timeoutMs` 的旧读法（同一前提的另外两处），§7.2 B-13 步骤列与 §8.4「脚本注入」行的命令形状写作裸 `bash <file>`。写法沿用 v0.3.7 / v0.3.8 已获认可的自律：**本版新增记账文本不复现被更正的三处旧串** ⇒ 裸 grep 即干净。不引入真实版号 / URL / 目标路径，不新增设计编号；按 v0.3.6 钉死的「对读基准版本」语义不替 PRD 回改锚点。逐处核对见 §17「v0.3.10 三处代码事实更正」自查表。
>
> **v0.3.11 的性质**：**v0.3.10 登记为「只报不改」的四处同类相邻滞后、经 Leader 重派（评论 `01a0d45d` 第 1 条）授权落文的续批——仍是机械更正：判定对象、编号、核对物零改动、不新增任何事实**。四处分两族，两族各自的同一事实都已在 §15.2「实现落点」行（v0.3.10）钉过，本批只是把余下四处拉回本文自己的口径，**不是新增知识**。① ② **第一族 = `timeoutMs` 的旧读法两处**：§1 引导句原把那枚 30 s 当作该通道对命令的兜底、§15.1 首行原以「命令通道默认超时」名之；实际 `timeoutMs` 只流向 `SshUtil:356` 的 `getOrCreateSession`（建连与认证），命令执行走 `SshUtil:368` 的 `executeRemoteCommand` 不带超时，两参重载的 `30_000L`（`FileAccessService:264`）因此是建连预算。§15.1 是「量级锚点」表、其取值 30 s 与出处一字未动，改的是标签与「这枚锚点能被当作什么用」——错着的标签会让下游把建连预算读成脚本耗时上限（§15.2「缺省值」行的推导即依赖它）。③ ④ **第二族 = 命令形状写作裸 `bash <file>` 两处**：§7.2 B-13 步骤列与 §8.4「脚本注入」行补齐为 `timeout <秒> bash <file>`，与已交付实现（`backend/core/.../deploy/ExtensionScriptRunner.java:96`）及两类适配器 `DEPLOY` 的既有形状（`DockerComposeAdapter:259-261`、`LinuxGsmDockerAdapter:222-223`）同形。两处更正**不改变 B-13 的核对物**（「命令文本里不出现脚本正文」「未校验通过不在宿主机留文件」逐字未动），也不新增判据：`timeout` 前缀带来的耗时语义一直由 §15.2 负责，本批只让形状在这两处写全。**登记同族表述三处仍不改（本版只报不改，交 Leader 判）**：§15.2「缺省值」行把 30 s 括注为「宿主机通道默认」（与本批更正后的 §15.1 标签不再一致，但其推导结论「缺省取 600 s 而非 30 s」不受影响）；§15.3 首行与 §12 RISK-D06 行的「`FileAccessService` 的超时语义是『不再等待』」——两处「不保证远端进程已终止」的结论仍成立（远端 `timeout` 发 SIGTERM，脚本派生的子进程可存活），但归因方式与本批口径不再一致，改它们要动 §15.3 与 RISK-D06 的登记语义 ⇒ 属需裁定处，不在本批授权面。写法沿用 v0.3.7 / v0.3.8 已获认可的自律：**本版新增记账文本不复现被更正的四处旧串** ⇒ 裸 grep 即干净。不引入真实版号 / URL / 目标路径，不新增设计编号；按 v0.3.6 钉死的「对读基准版本」语义不替 PRD 回改锚点。逐处核对见 §17「v0.3.11 四处同类相邻滞后」自查表。
>
> **v0.3.12 的性质**：**v0.3.11 登记为「只报不改」的同族表述三处、经 Leader 派单（评论 `01a0d484` §五-1 ①）授权落文的续批——仍是机械更正：结论、取值、编号、判据、登记对象零改动、不新增任何事实**。三处同一事实已在 §15.2「实现落点」行（v0.3.10）钉过：`timeoutMs` 只流向 `SshUtil:356` 的 `getOrCreateSession`（建连与认证），命令执行走 `SshUtil:368` 的 `executeRemoteCommand` 不带超时 ⇒ 「到点不再等待」与「不保证远端进程已终止」这两条语义的主体是**命令文本里那枚远端 `timeout`**（SIGTERM、退出码 124、脚本可 trap、子进程可存活），不是通道入参。**① §15.2「缺省值」行**：把 30 s 那枚括注的所指由「通道默认值」改到 §15.1 的**建连**预算，使本行与 v0.3.11 更正后的 §15.1 标签同源；该行取值 `600_000 ms` 与「取 30 s 会把正常的版本脚本判成超时」的推导结论逐字未动。**② §15.3 首行**：「不再等待」的所属由 `FileAccessService` 改指远端 `timeout`，并补上正确的失效机理（可 trap、子进程可存活）；新增的那枚实物指针只给类名不给行号，因为该类在 main 上尚不存在、而本文行号一律钉 main（§17 记这条口径）；本节的结论「**不保证远端进程已被终止**」、引导词「因此：」与其后三条子弹、以及「登记为 RISK-D06」一字未动。**③ §12 RISK-D06 行**：只给第 2 格的「脚本超时」指名是哪一枚，第 1/3/4 格与所引 §15.3 逐字未动 ⇒ 登记对象与缓解措施不变。**同族扫描的完整性**：本文 §1–§16 内「不再等待」命中恰 2 处（§15.3 首行、RISK-D06 行）、「通道默认」命中恰 1 处（§15.2 缺省值行）⇒ 本版后三处全部归因正确、正文内该族旧读法命中 0；相邻的 §15.2「超时后的判定」行只**引用 ui-spec 的既有词面**（`原因：脚本执行超过 〈timeoutMs〉 未返回，判失败`）、§14.13.3「命令」行本就写着 shell `timeout 1200` ⇒ 两行经核对**不属于本族、逐字未动**，不为改而改。写法沿用 v0.3.7 / v0.3.8 已获认可的自律：**本版新增记账文本不复现被更正的三处旧串** ⇒ 裸 grep 即干净（三处旧串在记账区的历史句里按「历史句不改写」口径原样保留，故归零断言按 §1–§16 正文范围做）。不引入真实版号 / URL / 目标路径，不新增设计编号；按 v0.3.6 钉死的「对读基准版本」语义不替 PRD 回改锚点、不替 UI 线回锚。逐处核对见 §17「v0.3.12 同族表述三处归因」自查表。
...
**命令通道两类，容器那一类不满足要求。** 宿主机 SSH 通道 `FileAccessService.executeCommand(hostId, command, timeoutMs)`（三参重载 `:254`；两参重载缺省传 `30_000L`，F-13）可用——但这里的「可用」只到「能把命令送出去、能拿回退出码」为止：`timeoutMs` 实际只流向 `SshUtil:356` 的 `getOrCreateSession`（建连与认证），命令执行走 `executeRemoteCommand` 不带超时（`SshUtil:368`）⇒ 那枚 30 s 是建一条通道的预算、不是脚本的耗时护栏，耗时护栏得写在命令文本上（口径见 §15.2「实现落点」行）；容器通道 `DockerComposeAdapter.executeCommand` 走 `compose exec`，**丢弃退出码、超时硬编码 60s、只返回输出文本**（F-16、§14.2 第三行），且 compose 类"停实例"= 容器停止，停后 `exec` 必然失败（`:395` vs `:728`）。
...
INIT → ENV_CHECK → PORT_CHECK → RESOURCE_CHECK → PRE_DEPLOY → DEPLOY
   ↓
【EXTENSION 阶段】（存在条件〔v0.3.5 按 Leader 裁定 01a0c4d8 与「入口判定即终止」一支对齐〕：解析出步骤集 ≥1，或虽无步骤集但扩展入口判定不合法、须按 BR-12 拦截（有 deployVersion 键 / 显式选择 ⇒ 阶段以「入口判定不合法」短暂成立：只产进入行 + 拦截行即 ERROR 终止，无步骤行、无收尾 / 完成 / 交棒行）；两者皆无则本段整体不存在，序列与今天逐字相同。仅 deployType ∈ {docker-compose, linuxgsm-docker} 允许声明，§14.13.1）
...
| D 版本选择 | 纯函数 `applyVersionSelection(configInfo, catalog)`（写/删/不写三态，§14.10；**v0.3.9 签名对齐**：第三参不设独立入参——用户所选版本只从载荷 `configInfo.deployVersion` 读，载荷即唯一真相（§6.1 的 `POST` 格已写「无新增字段：版本以 `configInfo.deployVersion` 承载」+ §16.4 末段））+ 提交期 BR-07 撞键校验（清单 = PRD 原三项，**`PLATFORM_IMAGE_TAG` 不进该清单**，其约束改由声明期保留键承担，§14.4.2）（**v0.3 订正**） | `backend/core` + `frontend` | 新增 |
...
§14.6 即 KPI-02 / AC-03 / AC-12 / AC-16 的核对契约，**以本节为登记处**：**六个字段位**（VO 上 8 个属性，SUG-18）、`stepEvent` 取值、**五条**归组与判定规则、耗时单位（`elapsedMs` 毫秒）、退出码承载位（`exitCode`）、`stdout`/`stderr` 的**非判据**地位、`stage = "EXTENSION"` 常量。UI 词面归 ui-spec §6.2（`docs/ui/MERC-3/ui-spec.md` @ `62b49a9`，v0.6 / S1b 定稿；并收状态只登记在元数据「平行输入」格、本文不另存第二份），两者关系：契约是判据，词面是渲染。
...
| B-10 | `DeployService`：`:214` 后插阶段（`DeployExtensionExecutor` 用已解析的 adapter，§14.13.2）；条件进度 `[80,84]` + 该分支内 `HEALTH_CHECK` 顶层 `progress` 起点 85（§14.7 订正：不是「进入行」）；`mapStageToStatus` 加 `EXTENSION → installing` | **无扩展声明游戏的路径**，进度值与阶段序列**逐值不变**（AC-15）；`HEALTH_CHECK` / `START` 代码未改一行；**`ensureRunningForExtension` 在无扩展部署中的调用次数 = 0**（V-10 新增子项） |
...
| B-13 | 脚本执行安全形状：正文/URL 脚本一律**平台侧下载 → sha256 校验（声明了才校验）→ SFTP 上传到 `<workDir>/.platform-extension/E-<n>.sh` → `timeout <秒> bash <file>` 执行（秒数 = 该步 `timeoutMs` 向上取整；护栏写在命令文本上，取形依据见 §15.2「实现落点」行）→ `finally` 删除** | 命令文本里不出现脚本正文（RISK-08 缓解）；未校验通过不在宿主机留文件 |
...
| 面 | 处置 |
| --- | --- |
| 脚本注入 | 正文**永不拼进命令行**（B-13 的落文件 + `timeout <秒> bash <file>` 形状：命令行里只有脚本文件名与秒数，没有正文），消除引号转义类注入面 |
| 补丁/脚本源 | 只接受 `http(s)`；声明了 `sha256` 即**先校验后落地宿主机**；`headers` 本期不可声明（§14.2） |
| 路径越界 | `targetPath` 绝对路径 / `..` → 声明不合法（BR-05） |
| 命令面扩大 | 声明仅出自插件 JAR 代码（非运行时可配），沿用宿主既有 SSH 凭证与容器控制权（§4.2「无新增权限位」）；`workDir` 下的 `.platform-extension/` 子目录仅放平台自建的临时脚本，`finally` 删该脚本文件、再尝试 `rmdir` 该目录（用 `rmdir` 而非 `rm -rf`：脚本自身往这里写过的产物不由平台顺手清空，目录非空则 `rmdir` 失败、原样保留，`ExtensionScriptRunner` 的 `deleteQuietly`） |
...
| 分支与 ADR 载体 | 实现统一走 `release/MERC-8-extension-steps-dnf-tw`；`docs/design/adr/0029-*.md` 与 `CONTEXT.md` / `glossary.md` 需从 `agent/leader/chat-1cfa252af963 @ 590af8d` **merge 带入**（远端可读） |
...
| V-08 | 日志呈现契约 | **机械核对脚本**：只读 `logs[].{stage,stepId,stepEvent,elapsedMs}` | **取值域 = 扩展阶段内的行**（v0.3.5 按 Leader 裁定 `01a0c4d8` 写死，**不再是「logs 全部行」**）：段内每条行 `stage == "EXTENSION"`，段外的既存阶段行不得取该常量——`INIT…DEPLOY` / `HEALTH_CHECK…COMPLETE` 各阶段行本就该是自己的阶段名，把全部 logs 行并进取值域会把一份正确实现判成违规，这正是本判据此前两种读法的分歧点。两侧的可执行形式（仍只读字段、不读 `message`）：`stepId != null` 的步骤行按构造只在段内产 ⇒ **必带** `"EXTENSION"`；`stepId == null` 的阶段级行按 §14.6 `stage` 行的钉值归属——**BR-12 拦截行在取值域内**（取 `"EXTENSION"`，其前必有一条同段的进入行）、**目录不合法说明行在取值域外**（取实际发生所在的既有阶段，本期 `"DEPLOY"`），脚本不因这两条 `stepId == null` 的条件行而误报。本项**不新增任何机械判据、不改任何阈值**，只把既有判据的取值域写死；**统计对象 = `stepId != null` 的行**——分母 = **非空** `stepId` 的去重计数（进入行 / 交棒行 / 停实例行 / BR-12 与目录不合法说明行 / **收尾行** / **阶段完成行**这些阶段级行 `stepId == null`，**一律不进分母**，v0.3.1 按 SUG-13 写死：它们按构造永不满足「恰一 START + 恰一终态」，含进分母则正确实现也只能得到 n/(n+1)，KPI-02 恒 < 100%）；每个非空 `stepId` 恰一 `START` + 恰一终态且终态 `elapsedMs != null`；比例 = 100%；**脚本内不得出现 `message` 匹配** | AC-03、AC-16、KPI-02、§14.6 规则 2 |
...
| V-12 | 健康判定路径 | 集成：两条部署各抓全量轮询样本（同一 deployType 内），V-27 负责跨类 | **有扩展步骤**：`HEALTH_CHECK` 行存在且**通过**（容器经扩展阶段收尾的 `adapter.ensureRunningForExtension` 起回），其后 `UPDATE_STATUS` / `START` / `retryHealthCheck` 与今天逐字同形；**无扩展声明（未触发 BR-12）**：整条序列不含 `EXTENSION` | §14.12、§14.13.3、AC-04、N-06/BR-10 |
...
| RISK-D06 | 脚本超时（= 命令文本里那枚远端 `timeout`，见 §15.2「实现落点」行）只保证「不再等待」，不保证远端进程已终止（§15.3） | 挂死脚本可能在宿主机继续跑，与重放叠加产生竞态 | 日志明示 + 声明侧幂等（BR-06）+ 本期不引入 kill 台账 |
...
| 字段 | 类型 | 契约项 | 约定 |
| --- | --- | --- | --- |
| `stage` | 既有 String | 阶段标识 | 扩展阶段全部行取常量 `"EXTENSION"`（与既有 `INIT/ENV_CHECK/…/COMPLETE` 均不冲突；该字段此前被前端完全丢弃，见 14.9）。**`stepId == null` 的两类条件行分别钉值**（v0.3.5 按 Leader 裁定 `01a0c4d8` 落文，两支不共用）：<br>**① BR-12 拦截行 ⇒ `"EXTENSION"`**——它发生在扩展阶段**入口**，该次部署的阶段自此以「入口判定不合法」短暂成立（§3.2 `:81` 存在条件）：产**进入行 + 本行**即 `ERROR` 终止，其后无步骤行、无收尾 / 完成 / 交棒行。⇒ §14.11 / F-02 的阶段带 latch 成立，与 ui-spec `62b49a9` 的 §4.2 `P`/`Q`（阶段带列 = 「渲染」）及 `LE` / `LF` 实物（带 √ + 进入行 √）同源。<br>**② 目录不合法说明行（§8.1「无键 → 默认版本 + 一条说明行」那一路）⇒ 它实际发生所在的既有阶段**，本期为 `"DEPLOY"`：该判定点位于 `DEPLOY` 完成行之后、`HEALTH_CHECK` 之前，而无键即不拦截、步骤集为空 ⇒ EXTENSION 段整体不存在（§3.2 `:81`），故本行承在已成立的 `DEPLOY` 上——`LG` 实物（带 × + 进入行 ×，ui-spec §13.3「⑧b」行把它归入「无阶段带：未进入扩展阶段」）即本格的判据来源。**实现若把读目录判定前移，本行随它前移后实际所在的既有阶段走**：钉的是「行归它实际发生的阶段」这条判据，不是 `"DEPLOY"` 这个字面量。<br>两行都 `stepId == null` ⇒ 无论取何值都**不进 KPI-02 分母**（规则 2 一字未动）；V-08 首判据取值域随之收敛，见 §10 |
...
| **B（推荐）** | 删键语义照样实现并有单测：`applyVersionSelection(configInfo, catalog)` 三态纯函数（**v0.3.9 签名对齐**：第三参不设独立入参，所选项只从载荷 `configInfo.deployVersion` 读）——选非默认 → 写键；选默认且键既存 → 删键；无目录 → 不写不删。AC-22 的判定改由「服务层三态 + `retry-deploy` 重放不丢键（AC-07 已覆盖）」承载，其**界面端到端**部分记「前提未成立」，回写 §8.4.2 / AC-22 各加一句限定 | 不扩范围；用户实际损失很小——今天想换回默认版本的真实做法就是新建一个默认版本实例，旧实例的键不再被任何入口读到 |
...
String holder   = "EXT:" + instanceId + ":" + seq;  // seq = 进程内自增；非任务 ID，取形依据见下表「holder 后缀的实际取形」行
...
| holder 后缀的实际取形（v0.3.10 按已交付代码更正） | 实测形状 = `EXT:<instanceId>:<进程内自增>`（`PatchInstallServiceImpl.installSync`，后缀取一个 `AtomicLong` 的递增值）。**上面的伪代码原取步骤序号作后缀，不成立**：`installSync` 的定稿 SDK 签名是 `(request, listener)` 两参，序号属调用方的呈现层概念（只作为 §14.6 的 `LogEntry` 字段位存在），补丁入口拿不到它。**后缀两路互不相同是正确性条件，不是风格**：`TaskMutexManager.putIfAbsent` 的防御支 `return existing.equals(taskId)`（`:52`）会让共用同一 holder 的两路都判「占用成功」，而 `finally` 里的 `remove` 是按值 CAS 删除（`:68`）⇒ 后进入的一路会把先进入那一路正持的键释放掉，互斥形同虚设；进程内自增即满足「两路不同」，无需把序号穿进 SDK。**前缀一字未动** ⇒ 上行的「不受 `removeByTaskId` 驱动」、本表「等待预算取 600 s」行、§10 V-26 的 holder 判据（只认前缀）取值与判定均不变 |
...
| 锚点 | 值 | 出处 |
| --- | --- | --- |
| 宿主机命令通道的**建连**预算（两参重载的字面量默认值，v0.3.11 按代码事实更正标签） | 30 s | `plugin/.../service/FileAccessService.java:264`（`executeCommand(hostId, command)` 重载的字面量 `30_000L`）——该值只流到 `SshUtil:356` 的 `getOrCreateSession`（建连与认证），命令执行 `SshUtil:368` 不吃超时 ⇒ 它是「建一条 SSH 通道」的量级、**不是**「一条命令最多跑多久」的量级，本表下游据此选脚本缺省值时不得当后者用 |
...
| 项 | 值 | 理由 |
| --- | --- | --- |
| **缺省值** | **`600_000 ms`（10 分钟）** | 扩展阶段的两类步骤必须处在同一量级：补丁步骤的既有 SSH 预算是 600 s，脚本取同值使「一步最多占用多久」对运维是**一个数**；取 30 s（§15.1 表内那枚通道的**建连**预算，不担保命令执行）会把正常的版本脚本判成超时，取 60 s（容器通道现状）无依据且本期该通道不用 |
...
| 实现落点（v0.3.10 按代码事实更正） | 执行通道 = `FileAccessService.executeCommand(hostId, command, timeoutMs)`（`:254`），但该入参**只兜住建连与认证**（`SshUtil:356` → `getOrCreateSession`）；命令执行走 `executeRemoteCommand`，**不带超时**（`SshUtil:368`）⇒ 它不构成脚本的耗时护栏。护栏在**命令文本**里：`timeout <秒> bash <file>`（远端 GNU `timeout` 发 SIGTERM、超时退出码 124），与两类适配器 `DEPLOY` 已在用的形状同形（`DockerComposeAdapter:259-261`、`LinuxGsmDockerAdapter:222-223`，两处的现场注释都逐字写着「`SshUtil` 的 `timeoutMs` 仅作用于建连，命令执行无超时」） | 通道（`SshUtil` / `FileAccessService`）一字未动 ⇒ 本期仍**零通道改造**，「行 3 容器通道本期不改不阻塞 OP-04」的结论不变；只是这格担保的是「不加新通道」，不担保「脚本被兜住」，后者自此由命令形状负责 |
| `timeoutMs` → 远端 `timeout` 的秒粒度（v0.3.10 细化） | 远端 `timeout` 只有秒的粒度 ⇒ **向上**取整（`ceil(timeoutMs / 1000)`，下界 1 s）：1400 ms → `timeout 2`。**超时识别按远端实际被杀的时刻比**，不按声明的毫秒数比：`exitCode == 124` 且 `elapsedMs >= 秒数 × 1000` 才判超时，判超时则 `exitCode` 归 `null` | ① 向下取整会在声明的预算还没用满前就把脚本杀掉（1400 ms → 1 s），这是主应用替插件推断并覆盖声明的一种，BR-04 禁止；向上的代价是预算最多被放宽不到 1 s，而本表「下限」行已把可声明预算钉在 `>= 1_000 ms`——下界 1 s 另防 `timeout 0` 被 GNU `timeout` 读成不限时。② 若按声明毫秒数比，一次真超时会被读成普通的非零退出 `124` ⇒ 破掉 §14.6 规则 4 要求的「超时与非零退出可靠区分」（超时 `exitCode == null`，原因段才填得对） |

### 15.3 一条必须登记的诚实限制

到点「不再等待」的是命令文本里那枚远端 `timeout`（§15.2「实现落点」行），不是 `FileAccessService` 的入参——`timeoutMs` 只兜建连、对命令执行没有超时可言。它发的是 SIGTERM、以退出码 124 收场，**不保证远端进程已被终止**：脚本可自行 trap 该信号继续跑，其派生的子进程也可能存活（已交付实现把这条写在 `ExtensionScriptRunner` 的类注释上）。因此：
...
**防回潮指针（v0.3.9，依据 = Leader 裁定评论 `01a0cf20` §四）**：**撞键校验只在提交期**（§3.3 组 D、§14.10 末段、§7.2 B-07、§10 V-03 四处同源），**N1…N5 刻意不含**「`deployVersion` 与 `variables[].name` 同名 ⇒ 目录不合法」这一条：补进来会给「目录不合法」多出一个判据来源，而本节的判据通道唯一读者就是 `DeployVersionCatalogService`。`prd.md` 一侧的同一口径已在同批出 v0.7（那版把 §8.4.1 / §8.4.3 / BR-07 与 §15 风险表对应行四处的「声明期与提交期都校验」统一为「提交期」，并在 §8.4.3 登记「为什么不补第六条声明期规则」）。⇒ 后来者再提「声明期也该拦撞键」时先回本指针；**要动 N1…N5 的项数须回头重跑 G1**——这五条是 G1 基线的一部分。
...
| **SUG-21**（G1-a ③） | **不硬改，登记为待裁定**（本轮 §14.6 `stage` / `stepId` 行与 §10 V-08 判据**一字未动**） | 本节（判定文本零改动） | 缺口核对为真：§14.6 `:673` 的七项枚举里，**BR-12 拦截行与目录不合法说明行确实没钉 `stage` 取值**，连带 §10 V-08 `:369` 首判据「每条 `stage == "EXTENSION"`」的取值域有两种读法。**但登记时给出的两支改法都与 v0.3.3 既有内容相撞**，且相撞方向两支不同：<br>① 取「两行都 = `EXTENSION`」一支 ⇒ 与 §3.2 `:81`「EXTENSION 段仅当解析出步骤集 ≥1 才存在，否则**本段整体不存在**」**及 §16.4 `:1045` 的 `INVALID` 态**冲突：目录不合法 + 无键那一路根本不进扩展阶段，给它的说明行钉 `EXTENSION` 会反向触发 §14.11 / F-02 的阶段带 latch（`logs.some(l => l.stage === 'EXTENSION')`），等于让一次「未进入扩展阶段」的部署长出阶段带——正是 AC-15 / AC-24 ③ 回归核对要禁的形态。<br>② 取「两行都 = 其实际发生的既有阶段」一支 ⇒ 与 **BR-12 拦截行的实物**冲突：ui-spec §4.2 的 P / Q 两行明写「阶段带 **渲染**」，对应屏 LE / LF 的日志序列含「进入部署扩展阶段」且在阶段内终止（ui-spec §6.2 规则① 称其为「**扩展阶段入口**的 BR-12 拦截」）⇒ 该行的实际发生阶段就是 `EXTENSION`，改称既有阶段会让 latch 与 AC-16 的判读反向落空。<br>**核对用的实物证据（对 `ui-spec/index.html @ 62b49a9` 逐屏机械抽取，命令随回传）**：`LE` `LF` = 带 √ + 进入行 √；`LG` = 带 × + 进入行 ×。<br>**因此两行必须分别钉、不能共用一支**：BR-12 拦截行 ⇒ `stage = "EXTENSION"`（阶段以「入口判定不合法」短暂成立，latch 与实物一致）；目录不合法说明行 ⇒ `stage = 其实际发生所在的既有阶段`（本期该点位于 `DEPLOY` 完成行与 `HEALTH_CHECK` 之间 ⇒ `"DEPLOY"`；实现若把读目录判定前移，则为其前移后实际所在的既有阶段，判据不变），并在同一句里把 V-08 首判据的取值域写明为「扩展阶段内的行」而非「logs 全部行」，使脚本不在这两行上误报。**该拆分是否采纳、以及 `INVALID` 说明行的正钉值，交 Leader / @Architect 裁定；裁定后单独一批落文 §14.6 与 §10 V-08。**<br>**影响面复核（与登记一致）**：KPI-02 上为 **0**——两行 `stepId == null`，无论 `stage` 取何值都不进分母；风险只在 V-08 脚本的可执行歧义与 AC-20 / BR-12 的验收归因。**连带发现（一并交裁定）**：LE / LF 在零步骤的情况下渲染阶段带，而 §3.2 `:81` 把 EXTENSION 段的存在条件写成「解析出步骤集 ≥1」——这两处本身也有一格张力（入口拦截算不算「进入过该阶段」），SUG-21 落文时应顺手把 `:81` 的存在条件与「入口判定即终止」这一支对齐，避免同一问题在 §3.2 与 §14.6 两处各修一半。**〔v0.3.5：Leader 裁定 `01a0c4d8` 采用本行末尾这套拆分（不选单支、不挂实现期），三处判定文本已按此落文——`§14.6 stage 行 / §10 V-08 首判据取值域 / §3.2 :81 存在条件`，逐处见下「v0.3.5 裁定落文（SUG-21 闭合）」块；本行按「历史行不改写」口径原样保留〕** |

**v0.3.5 裁定落文（SUG-21 闭合）+ 相邻面核对（Leader 裁定评论 `01a0c4d8`：采用拆分，不选单支、不挂实现期）**：

按 v0.3.4 登记的拆分**单独一批、单独 commit** 落文（「三件同批」在 ③ 上做不到 ⇒ 零轮次记账延续，不占评审轮次）。裁定依据即 v0.3.4 给出的两组实物：统一取 `EXTENSION` 被 `LG`（带 × + 进入行 ×）证伪，统一取既有阶段被 `LE`/`LF`（带 √ + 进入行 √）证伪。

| # | 处置 | 落点 | 说明 / 核对到的事实 |
| --- | --- | --- | --- |
| **SUG-21 ① BR-12 拦截行** | **落文：`stage = "EXTENSION"`** | §14.6 `stage` 行 ① 段 | 阶段以「入口判定不合法」短暂成立：产进入行 + 拦截行即 `ERROR` 终止 ⇒ §14.11 / F-02 的 latch 与 ui-spec `62b49a9` §4.2 `P`/`Q`（阶段带列「渲染」）、`LE`/`LF` 实物三处同源。**与 §14.6 `stage` 行原句「扩展阶段全部行取常量 `"EXTENSION"`」不构成改动**——本段只是把「哪几行算扩展阶段的行」写实 |
| **SUG-21 ② 目录不合法说明行** | **落文：`stage = 实际发生所在的既有阶段`，本期 `"DEPLOY"`** | §14.6 `stage` 行 ② 段 | 该判定点在 `DEPLOY` 完成行之后、`HEALTH_CHECK` 之前，且无键即不拦截、步骤集为空 ⇒ 段不存在（§3.2），行承在已成立的 `DEPLOY` 上。**钉的是「行归它实际发生的阶段」这条判据、不是 `"DEPLOY"` 字面量**：实现若前移读目录判定，行随之取其前移后实际所在的既有阶段，判据不变（裁定原话） |
| **SUG-21 ③ V-08 首判据** | **落文：取值域写明「扩展阶段内的行」** | §10 V-08 判据格 | 原字面「每条 `stage == "EXTENSION"`」有两种读法，按 logs 全部行读会把一份正确实现判成违规（既存阶段行本就该取各自的阶段名）。收敛后两侧仍只读字段：`stepId != null` 的步骤行必带该常量；`stepId == null` 的阶段级行按 §14.6 钉值归属，BR-12 拦截行在域内、目录不合法说明行在域外。**不新增机械判据、不改任何阈值**；KPI-02 的「统计对象 / 分母 / 比例 = 100% / 脚本内不得出现 `message` 匹配」四段逐字保留（见下表核对） |
| **SUG-21 连带项** | **落文：§3.2 `:81` 存在条件两支化** | §3.2 `:81` 一行 | 裁定书面授权同批（「同一缺陷，拆开＝各修一半」）。原条件「仅当解析出步骤集 ≥1 才存在」与 `LE`/`LF` 实物冲突（该两屏零步骤却渲染带），现补第二支：无步骤集但入口判定不合法须 BR-12 拦截 ⇒ 段短暂成立、只产进入行 + 拦截行。**改动严格限于这一行的括号内**；`:82` 段内序列与 `:83` 终止条件一字未动 |
| **相邻面核对：三行不矛盾** | **未动，仅登记** | §7.3 F-02 `:248`、§10 V-10 `:371`、§14.7 `:712`（另 §14.13.2 `:842` 同族措辞一并扫过） | 这些行的主语是「**无扩展部署** / **无扩展** / **无扩展步骤（本期绝大多数游戏）** / **无步骤的游戏**」= 无扩展声明的那批游戏。BR-12 触发的前提恰是「有目录声明 + 有 `deployVersion` 键或显式选择」，与「无扩展声明的游戏」互斥 ⇒ 本裁定不改这几行的判定对象。dnf-tw 本期态 `EMPTY`（§16.4 `:1044`）明写「不产生任何「不合法」提示行」、不拦截，AC-24 ③ 的回归对象因此仍落在「不渲染阶段带」一侧 ⇒ 复核为真、未动 |
| **相邻面核对：两行字面被本裁定读宽** | **超授权面 ⇒ 未改，报 Leader 定** | §7.2 B-10 `:230`、§10 V-12 `:373` | 两处的判据对象仍是 AC-15 那批无扩展声明的游戏、要判的东西一字不变，但**字面**用的是「无步骤路径的进度值与阶段序列逐值不变」/「**无扩展步骤**：整条序列不含 `EXTENSION`」——BR-12 那一路零步骤而序列含两条 `EXTENSION` 行，字面上会被罩进去，是 @Tester 落脚本时真会踩的一格。**最小改法（各一处，不在本版自行落文）**：V-12 该短语改「**无扩展声明（未触发 BR-12）**」；B-10 的「无步骤路径」改「**无扩展声明游戏的路径**」。Leader 若判「不补」，本表这两行即为该判断的记录处；若判补，属另一次自然触手，不占本版。**〔v0.3.6：Leader 在裁定评论 `01a0c512` 第 ① 条判「补」，本行末给出的两处最小改法已按原样落文**——B-10「无步骤路径」→「无扩展声明游戏的路径」、V-12「无扩展步骤」→「无扩展声明（未触发 BR-12）」；两处改的是主语的口径、判据与核对物一字未变，本表上一行点名的三行（F-02 / V-10 / §14.7）照旧未动。本行按「历史行不改写」口径原样保留，此括号是授权与落文的指针。**〕** |
...
**v0.3.5 自查（三处落文的边界核对，全部可复跑；核对脚本随回传评论附上）**：

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| 裁定三处确实落文，且判定文本**只**动这三处 | 复跑核对脚本 `selfcheck-v035.py --old c8a6f3f --new <本版>`，组 (1)(3)。锚点一律按**行首形态**取、不按子串（本版 §17 新写的文字里原文引用了 §3.2 `:81` 与 §14.6 的既有句，子串粗筛会多命中）。**本批行号约定**：下表所有 `:NNN` 一律指 **`c8a6f3f`（v0.3.4）** 的行号；本版在 §0 性质注处插入 2 行，故 `:24` 之后各行在本文件内整体 +2（`:81`→`:83`、`:369`→`:371`、`:230`→`:232`…）——脚本按行首形态锚定、不按行号锚定，因此偏移不影响任何一条判据 | **(1) 9 项全 PASS**：BR-12 拦截行钉 `EXTENSION` / 目录不合法说明行钉既有阶段且本期 `"DEPLOY"` / 三处均带裁定号 `01a0c4d8` / V-08 写明「扩展阶段内的行」并明确否定 logs 全部行、旧字面「每条 `stage == "EXTENSION"`；」不再以行首形态存在 / `:81` 两支化且保留「两者皆无则本段整体不存在」/ 三处与 `c8a6f3f` 逐一不同。**(3) 22/22 处逐字未动**：§14.6 `stepId` 行、§14.6 规则 2、§14.6 `stepEvent` 行、§14.6 `elapsedMs` 行、§16.4 `INVALID` 行、§16.4 `EMPTY` 行、§14.11 小节标题、§14.11 P3 latch 行、§3.2 `:82` 段内序列、§3.2 `:83` 终止条件、§7.2 B-10、§7.3 F-02、§10 V-10、§10 V-12、§14.7 无扩展步骤行、§14.13.2 无扩展部署落点行、§18 的 v0.3.4 / v0.3.3 / v0.2 三条历史行、元数据「上游 PRD」/「上游 ADR」/「平行输入」三行 |
| **尺子本身不空转**（同一把尺子跑「按裁定只登记、未落文」的那一版必须 FAIL） | `--old ee65e66 --new c8a6f3f`（v0.3.3 → v0.3.4，正是 SUG-21 未落文的那一批） | **exit 1、FAIL 11 项 = 组 (1) 的全部 9 项 + 组 (3) 的两项**；组 (2)(4) 全过。组 (3) 那两项是 v0.3.4 **自身的合法改动面**（「平行输入」行正是 G1-b 要改的那一格；§18 的 v0.3.4 行在 `ee65e66` 还不存在，属锚点定义性缺失、不是回归）⇒ 剔除这两项后该对照组的组 (3) 为 **20/20 未动**。结论：本版的 PASS 是「真落了文」的结果，不是判据太松 |
| KPI-02 口径一字未动 | 组 (2)：把 SUG-13 建立的七段字面逐个断言仍在本版 V-08 行内（统计对象 / 分母 = 非空 `stepId` 的去重计数 / 恰一 `START` + 终态且 `elapsedMs != null` / 比例 = 100% / 一律不进分母 / 脚本内不得出现 `message` 匹配 / KPI-02 恒 < 100%），**并反向断言同一批字面在 `c8a6f3f` 的 V-08 行里同样在**（防脚本样本自身漂移）；规则 2 的分母串另起一条断言 | 七段全在、规则 2 未动 ⇒ 本版对 KPI-02 的分子与分母**零改动**，与 §17 登记的影响面「KPI-02 = 0」一致。V-08 行新增的文字全部只谈**取值域**，未触碰任何阈值或统计口径 |
| 改动面 | 组 (5)：`git diff -U0 c8a6f3f -- docs/design/MERC-3/design.md` 的 hunk 清单 | **9 个 hunk**，新增侧起始行 `7,15,24,83,371,674,1147,1222,1246` = 元数据「版本 / 状态」两格、版本链一句、v0.3.5 性质注、§3.2 `:81`、§10 V-08、§14.6 `stage` 行、§17 登记表（SUG-21 指针 + 裁定落文表）、§17 本自查块、§18 v0.3.5 行 ⇒ **判定文本恰三处**（`:83` / `:371` / `:674`，即 `:81` / V-08 / `stage` 行），其余六处全是记账。触及文件只有本文件（`prd.md` 命中 0、`docs/ui/MERC-3/*` 命中 0、功能代码 0）；PRD 的回写按 Leader 裁定第 3 条单独成批 |
| 表格结构与编号 | 组 (4)：剥 code-span 后统计列数的 lint（验法与 v0.3.3/v0.3.4 同一套，绝对数以本版实测为准）；对 `c8a6f3f` 与工作版各抽 `V-`/`D-P`/`RISK-D`/`B-`/`AC-`/`T-`/`F-`/`BR-`/`FR-`/`KPI-`/`OP-`/`D-N`/`N`/`G-`/`SUG-`/`REV-` 编号集合做对称差 | 表格 **69 张**（v0.3.4 为 67，本批新增 §17 两张：裁定落文表 + 本自查表）、非分隔第二行 **0**、列数不一致 **0**；编号对称差为**空**（双向均无新增）⇒ 本版不另造编号空间，SUG-21 用既有号闭合；两行相邻面的「最小改法」按超授权面处理、不占用编号 |
| 禁编造复扫（与 v0.2…v0.3.4 同口径） | 公网 URL 字面量、`\d+\.\d+\.\d+` 形态串集合、dnf-tw 真实版号片段各扫一遍，并与 `c8a6f3f` 的集合做差 | URL **0**；`x.y.z` 集合与 v0.3.4 **完全相同、新增 0**（命中仍全为节号、jar 名、`javac 17.0.16`、`docker-java 3.3.4` 四类既有项）⇒ 本批钉的是**阶段常量的取值**，未引入任何真实版号 / URL / 目标路径 |
| 与 UI 实物的一致性（不靠本批自证） | 对 `ui-spec.md @ 62b49a9` 复核两处**已定稿**文字：§4.2 `P`/`Q` 行的阶段带列、§13.3「⑧b」行末段对 `LB` / `LG` 的归类 | ① `P`/`Q` 两行阶段带列 = 「**渲染**」，且同一节称 `LE`/`LF` 为「入口拦截、**零步骤**」⇒ 与本版把 BR-12 拦截行钉 `EXTENSION` 同源；② 同一行把 `LB` / `LG` 归入「**无阶段带**、与本条无关的屏：未进入扩展阶段」⇒ 与本版把目录不合法说明行钉在其实际发生的既有阶段同源。**两处都是 UI 侧 v0.6 已定稿的文字：本版不需要 UI 回改，UI 也不需要反过来追认本设计** |
| 边界 | `git diff --name-only c8a6f3f HEAD` | 仅命中本文件；未碰 `prd.md`（回写批另成 commit）、未碰 `docs/ui/MERC-3/*`、未碰功能代码；新 commit 续在 `c8a6f3f` 之后，**不覆盖、不 force-push** |

**v0.3.6 自查（两处授权补项 + 一处锚点统一，全部可复跑；核对脚本随回传评论附上）**：

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| 三行授权改动确实落文，且判定文本**只**动这三行 | 复跑 `selfcheck-design-v036.py --old 347faf3 --new <本版>`，组 (1)(6)。锚点一律按**行首形态**取、不按子串（本版新写的文字里原文引用了 B-10 / V-12 的旧字面作为「被订正对象」，子串粗筛会把它们误判成残留）。**本批行号约定**：下表 `:NNN` 一律指 **`347faf3`（v0.3.5）** 的行号；本版在 §0 性质注处插入 2 行，故 `:232`→`:234`、`:375`→`:377`；脚本按行首形态锚定、不按行号锚定，偏移不影响任何一条判据 | **组 (1) 17 项全 PASS、组 (6) 判定文本恰 3 行**：三行各「新文本逐字在位」+「旧字面不再作现行断言」+「与 `347faf3` 确为不同文本」（第三项防「判据空转」），另逐串断言这三行的**其余部分未被顺手动过**（B-10 的「`HEALTH_CHECK` / `START` 代码未改一行」与 `ensureRunningForExtension` 调用次数子项、V-12 的「`HEALTH_CHECK` 行存在且通过」「与今天逐字同形」「§14.12、§14.13.3、AC-04、N-06/BR-10」追溯列、PRD 行的语义钉法三句） |
| **改的是主语的口径、不是判据**（裁定 ① 的口径要求） | `difflib` 词级比对 `347faf3` 与本版这两行的整行差异，逐段点名 | B-10 词级差**恰一段**：`无步骤路径的` → `无扩展声明游戏的路径，`（另含该短语的两枚加粗标记）；V-12 词级差**恰一段**：`无扩展步骤` → `无扩展声明（未触发 BR-12）`。两行其余全文（含要判的东西：「进度值与阶段序列逐值不变（AC-15）」/「整条序列不含 `EXTENSION`」、以及各自的追溯列）**一字未变** ⇒ 核对物不增不减，符合「判据对象仍是 AC-15 那批游戏、要判的东西不变」的登记口径 |
| 其余未授权锚点逐字未动（Leader 核对口径的第二半） | 组 (2)：20 处锚点在 `347faf3` 与本版各按行首形态唯一命中，再逐字比对整行 | **20/20 PASS**——§14.6 `stepId` / 规则 2 / `stepEvent` / `elapsedMs` 四处、§16.4 `INVALID` 与 `EMPTY` 两行、§14.11 小节标题与 P3 latch 行、§3.2 段内序列行与终止条件行、§7.3 F-02、§10 V-10、§14.7 无扩展步骤行、§14.13.2 无扩展部署落点行、元数据「上游 ADR」与「平行输入」两行、§18 的 v0.2 / v0.3.3 / v0.3.4 / v0.3.5 四条历史行。与 v0.3.5 那张自查表的 22 项清单对账：`22 − 本版授权 3 = 19`，再加本版新增为历史行的 **§18 v0.3.5 行**，恰 **20** ⇒ 与 `01a0c512` 「其余 20 行」同数同集 |
| 上一版的三处钉值未被回动、KPI-02 口径未动 | 组 (3)：§14.6 `stage` 行、§10 V-08 首判据、§3.2 存在条件三处整行比对；KPI-02 的七段字面在 **§1–§16 判定文本区**的命中数逐段比对（这七段散布于 V-08 行、§14.6 规则 2 与 §17，故按命中数而非单行判；§17/§18 记账区排除在外，否则本版自查行的原文引用会自证污染） | **三处逐字未动**；**七段命中数逐段相等**（§1–§16 内依次数 `1 / 1 / 2 / 1 / 2 / 1 / 2`——只数判定文本区，因为本版 §17 自查行原文引用了这七段，按全文数会自证污染）⇒ 本版对 v0.3.5 的裁定落文零回退 |
| **尺子本身不空转** | 同一脚本跑两组对照：`--old 347faf3 --new 347faf3`（未落文的基准）与 `--old c8a6f3f --new 347faf3`（上一批自身） | 前者 **exit 1、FAIL 13 项 = 组 (1) 17 项中的 12 项 + 组 (6) 的「恰三行」**（组 (1) 余下 5 项是「这三行的其余部分未动」的断言，基准版自证为真）⇒ 本批判据判的是本版才成立的事实；后者 **exit 1、FAIL 19 项**（组 (1) 12 + 组 (2) 1 即 §18 v0.3.5 历史行彼时不存在 + 组 (3) 3 即 v0.3.5 自身钉的三处 + 组 (6) 1）。两组都 FAIL 而本版 PASS ⇒ PASS 不是判据太松 |
| 锚点统一「不回改 PRD」的连带面（裁定 ② 的另一半） | 读 `prd.md @ ee86825` 元数据「v0.6 回写来源（锚点）」行的现行取值；再 `git diff --name-only 347faf3 HEAD` | 该行现锚本设计 **`347faf3`（v0.3.5）**——本版不回改它，且这是**裁定要求的**而非遗漏：`01a0c512` 第 ② 条把锚点语义钉为「对读基准版本」、metadata-only 升版不触发反向回改 ⇒ PRD 侧不必回锚本版，本设计也替它不动。`prd.md` **零改动**（不在 diff 名单内）⇒ 两文件自此不互相追版本 |
| 表格结构与编号 | 组 (4)：剥 code-span 后统计列数的 lint（验法与 v0.3.3 起同一套，绝对数以本版实测为准）；对 `347faf3` 与本版各抽 `V-`/`B-`/`AC-`/`T-`/`F-`/`BR-`/`FR-`/`KPI-`/`OP-`/`D-P`/`D-N`/`N`/`G-`/`RISK-D`/`SUG-`/`REV-`/`NT-`/`MF-`/`SG-`/`L-` 编号集合做对称差 | 表数 `69 → **70**`（本批 +1，即本自查表）、**列数不一致 0 张**、**缺分隔行 0 张**；编号**对称差为空** ⇒ 本版不引入任何新设计编号，只引用既有编号与评论号 `01a0c512` |
| 禁编造复扫（与 v0.2…v0.3.5 同口径） | 组 (5)：公网 URL 字面量、`x.y.z` 形态串集合、dnf-tw 真实版号片段各扫一遍并与 `347faf3` 做差；**版本号字面量与「真实资料」不同源**，比对前先按 `v\d+\.\d+\.\d+` 记法剥离（否则本版新写的 `v0.3.6` 会被自己抓走） | URL **0**；`x.y.z` 集合与 `347faf3` **完全相同、新增 0**（命中仍全为节号、jar 名、`javac 17.0.16`、`docker-java 3.3.4` 四类既有项）；剥离后新增的版本号字面量集合只含 **`v0.3.6`** 自身。⇒ ① 补的是**主语的口径**、② 补的是**commit 号**，两类都不引入资料 |
| 边界 | `git diff --name-only 347faf3 HEAD`；`git log` 关系 | 仅命中本文件；未碰 `prd.md`、未碰 `docs/ui/MERC-3/*`、未碰功能代码；新 commit 续在 `347faf3` 之后**同一分支**（`agent/architect/merc-3`），**不覆盖、不 force-push** |

**v0.3.7 自查（交付分支名统一批，全部可复跑；核对脚本与完整输出随回传评论附上）**：

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| 旧名**全量**收净（授权原话是「一次 grep 收干净」，不是「按点名清单改」） | 对 `790ccd6` 与本版各数被替换分支名整串的命中行数与出现次数；再对本版扫 `release/` 前缀的全部命中，逐行确认有无第二形态的落点（简写、去 `release/` 的裸名、英文写法）；另确认 ADR 文件名的字面**未被牵连**（它是另一枚既有文件名，不是被替换的那个值，替换前后命中数相等） | `790ccd6`：命中 **2 行 / 恰 2 次**（`:13` 元数据「交付分支」格、`:338` §9.1「分支与 ADR 载体」行）→ 本版：**0 行 / 0 次**（判据 (1b)）。本版新名的命中位置 = **那两枚替换点**（`:13`、`:338`→本版 `:340`）**+ 本版新写的记账文本**（元数据状态格、版本链一句、§0 性质注、本节、§18 本版行）；**其中落在 §1..§16 判定正文区的恰 `:340` 一行**（判据 (1c) 机器核的正是这条）。简写形态、去前缀的裸名形态、英文写法形态各 **0**（判据 (1e)）；ADR 那个文件名的字面命中数新旧**相等**（判据 (1f)，它是另一枚既有名字，不是被替换的那个值）⇒ 授权说的「若另有落点一并」**实测没有另有落点**，两枚替换点即全量。**本批行号约定**：本表与本版记账里的 `:NNN` 一律指 **`790ccd6`（v0.3.6）** 的行号；本版在 §0 性质注处插入 2 行，故 `:338`→本版 `:340`、`:210`→本版 `:212`——核对一律按**整行内容 / 行首形态**锚定、不按行号锚定，偏移不影响任何一条判据。**另刻意的写法**：本版所有记账文本**一律不复现被替换的那个旧值字面串**，只以「MERC-3 时期拟定的旧名」指代（本节亦不外溢：旧值命中数是对**全文**数的（判据 (1b)），记账区自然含在内）⇒ 与 v0.3.6 那批「旧字面仍出现在带引号的订正说明里、要靠『不作现行断言』来判」不同，本批**任何一次裸 grep 都是干净的**，无需区分订正说明与断言 |
| 两处都是**纯值替换**、未夹带语义（授权边界「凡需改语义处停下先报」的核对） | 按行首前缀把本版这两行与 `790ccd6` 对应行配对（各须恰一行命中），逐行做**整行等式断言** `旧行.replace(旧名, 新名) == 新行`——等式成立即等价于「除该子串外一字未动」，比词级 diff 更强、且不需要人工数段落 | 两行等式**均成立**（`:13`、`:338`→本版 `:340`）⇒ 差**各恰一段且仅一段**。「交付分支」格仍是「一行一值」的裸锚点格，**未追加起点 / 顺序 / 载体等任何新断言**；`:338` 行携带的另一件事——`docs/design/adr/0029-*.md` 与 `CONTEXT.md` / `glossary.md` 需从 `agent/leader/chat-1cfa252af963 @ 590af8d` **merge 带入**（远端可读）——由等式保证一字未动，且在新名下复核仍为真（`590af8d` 仍是 ADR-0029 的载体、实现分支仍须 merge 带入它）⇒ **停止条件未触发，本版没有任何一处需要改语义** |
| **判定文本恰零改动**（本批比 v0.3.4 更窄：连一处口径都不动） | **不用抽样清单**，用整文件多重集差：`Counter(790ccd6 全部行) − Counter(本版全部行)` 与反向差，把「被改写的既有行」全集求出来再逐行分类；纯插入行不参与「改写」计数 | 被改写的既有行**恰 5 行**：元数据「版本」`:7`、「状态」`:8`、「交付分支」`:13`、版本链 `:15`、§9.1 `:338`。前四行全是记账，**落在正文区的恰 1 行**（实测 `§1..§16` 区间内的改动集合 == `{338}`），而该行改的是分支名的字面值、不触碰任何判据。⇒ **判据 / 口径 / 钉值的改动数 = 0**。`790ccd6` 全文 1263 个正文行，其余 **1258 行在本版逐字保留**；本版净增行数全部是**纯插入**（性质注 2 行 + 本节自查块 + §18 本版一行），插入不改写任何既有文字 |
| 上一批与上上批的钉值零回退（不另列清单，由全量证明覆盖） | 上一行的「其余 1258 行逐字保留」已**穷尽**任何抽样：v0.3.5 钉的三处（§14.6 `stage` 行 / §10 V-08 首判据取值域 / §3.2 存在条件）、v0.3.6 落文的三行（§7.2 B-10 / §10 V-12 / 元数据「上游 PRD」）、KPI-02 的分子分母口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、§18 的 v0.1…v0.3.6 历史行、元数据「上游 ADR」「平行输入」两行**必然在内**。另对 KPI-02 的七段字面在 `§1..§16` 区间做命中数逐段比对（`§17`/`§18` 排除在外，否则本节引用自身会自证污染） | **零回退**（由全量保留直接推出，不依赖抽样是否选得准）；七段命中数新旧**逐段相等**（依次数 `4 / 4 / 11 / 6 / 1 / 15 / 2`）。**注**：这七串是本脚本自定的验法，与 v0.3.6 自查所记 `1/1/2/1/2/1/2` 不同源、绝对数不可直接比大小；两版各自的判据都是「**同段新旧相等**」，不是「等于某个绝对数」 |
| **尺子本身不空转** | 同一脚本跑对照组 `--ref 790ccd6`（即本版父提交自身，未落文） | **exit 1、FAIL 10 项 / PASS 12 项**，逐项点名：**(1b)** 旧名归零（基准侧仍命中 2 行 2 次）、**(1c)** 新名落在判定正文的恰一行、**(1d)** 新名在两枚替换点在位、**(2)** 两行的整行等式（替换未发生）、**(3a)(3f)(3g)(3d)** 被改写既有行集合与「全为纯插入」（基准侧净增 0、改写集空）、**(5a)** 表数 +1（基准侧 70→70）。⇒ 本版 PASS 的 23 项里，这 10 项在父提交上**不成立**——本批判据判的是本版才成立的事实，PASS 不是判据太松 |
| 表格结构与编号 | 剥 code-span 后统计列数的 lint（验法与 v0.3.3 起同一套，绝对数以本版实测为准）；对 `790ccd6` 与本版各抽 `V-`/`B-`/`AC-`/`T-`/`F-`/`BR-`/`FR-`/`KPI-`/`OP-`/`D-P`/`D-N`/`N`/`G-`/`RISK-D`/`RISK-`/`SUG-`/`REV-`/`NT-`/`MF-`/`SG-`/`L-` 编号集合做对称差 | 表数 `70 → **71**`（本批 +1，即本自查表）、**列数不一致 0 张**、**缺分隔行 0 张**；编号**对称差为空**（新旧各 187 个）⇒ 本版不引入任何新设计编号，新增标识符只有引用到的评论号 `01a0c526` / `01a0c540` / `01a0c552` 与新分支名本身，均不进入上述抽样 |
| 禁编造复扫（与 v0.2…v0.3.6 同口径） | 公网 URL 字面量、`x.y.z` 形态串集合、dnf-tw 真实版号片段各扫一遍并与 `790ccd6` 做差；版本号字面量与「真实资料」不同源，比对前先按 `v\d+\.\d+\.\d+` 记法剥离 | URL **0**；`x.y.z` 集合与 `790ccd6` **完全相同、新增 0、移除 0**（剥离前后一致，命中仍全为节号、jar 名、`javac 17.0.16`、`docker-java 3.3.4` 四类既有项）；剥离后新增的版本号字面量只含 **`v0.3.7`** 自身 ⇒ 本批换的是一个**分支名字面值**，不引入任何真实版号 / URL / 目标路径 |
| 边界 | `git diff --name-only 790ccd6 <本版>`；`git log --oneline 790ccd6..<本版>`；`git ls-remote` 与父提交的快进核对 | 仅命中本文件；未碰 `prd.md`、未碰 `docs/ui/MERC-3/*`、未碰功能代码；新 commit 续在 `790ccd6` 之后**同一分支**（`agent/architect/merc-3`）、**恰一次提交**、**不覆盖、不 force-push**（metadata-only 升版按 v0.3.6 钉死的「对读基准版本」语义不触发反向回改，故 PRD 不回锚本版） |
| **本批暴露的相邻滞后（只登记、不改：超出「分支名替换」授权面）** | 扫本文全部指向 UI 产物的锚点，看除已统一的元数据「平行输入」格外还有无别处仍指老 commit | `§6.3` 的「UI 词面归 ui-spec §6.2（`docs/ui/MERC-3/ui-spec.md` @ `f6312ef`，S1b v0.4 在途）」（`:210`）仍指 **`f6312ef`（UI v0.1）**，而元数据「平行输入」格已在 G1-b 统一至 `62b49a9`（v0.6）⇒ **同一文件内两个 UI 锚点不同源**。**本批未改**：它不是分支名，且改它须连带判「S1b v0.4 在途」这半句在 v0.6 下的现行真假（UI 是否已随 `62b49a9` 定稿，本文除「平行输入」格外没有依据），属改语义 ⇒ 按授权边界**停下先报**，交 Leader 定或留作下一次自然触手。**〔v0.3.8：本行当时缺的那条依据已由 Leader 评论 `01a0c56e` 给齐——元数据「平行输入」格自身即把 `62b49a9` 读作 v0.6 / S1b 定稿，加上 G1 已通过 `01a0c540` ⇒ 授权单开一批。本版已按该授权的 A 条落文（锚点统一 + 同源指针），本行末段说的「不同源」自此闭合；本行按「历史行不改写」口径原样保留，「本批未改」指 v0.3.7 那一批〕** |

**v0.3.8 自查（§6.3 UI 锚点同源批，全部可复跑；核对脚本与完整输出随回传评论附上）。本表 `:NNN` 一律指**父提交 `056ae71`** 的行号（沿用 v0.3.6 立的约定）：本版在 §0 性质注处插入 2 行、§17 表末插入 17 行、§18 插入 1 行，故 `:212`→`:214`、`:1223`→`:1225`、`:1265`→`:1267`；脚本一律按行首形态锚定、不按行号锚定，偏移不影响任何一条判据**：

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| 旧锚点在**现行断言区**归零（授权判据原话：现行锚点集合里旧锚点命中 **0**） | 分三层数，不混成一个数：① **§1–§16 判定正文区**（本版定义为 `## 1.` 行起、`## 17.` 行前）内旧锚点整串的命中行数与出现次数；② **元数据表**（`:3`–`:13`）内「`@` + 反引号包裹锚点」这种**锚点槽位形态**的命中数，与普通字面命中分开数；③ 全文其余每一处命中所在行，逐行确认落在 §0 性质注 / §17 / §18 / 元数据 四类**记账区**之内 | ① **0 行 / 0 次**（判据 (1a)；父提交 `056ae71` 侧为 **1 行 / 1 次**，正是 §6.3 那行）⇒ 判定正文自此无任何一处指向 UI v0.1。② **0**：元数据「平行输入」格的锚点槽位是 `62b49a9`，旧锚点只出现在「随 UI 定稿自…统一」的**迁移从句**里，不占槽位。③ 剩余 **6 行 / 7 次**命中全部在记账区，**本版新增 0 处**（本版新写文本一律以「UI v0.1 那枚旧锚点」指代） |
| 剩余每处命中的**读法限定**（授权判据原话：历史叙述里若提它，必须带旧锚点限定） | 按**行**判（该行任一命中带 `（v0.1）` / `（UI v0.1）` 版本限定即算通过，因命中常在句末而引用点在句中的旧写法里），不含者单独点名并给出它在句中实际承担的语境，不静默放过 | **5 行**带版本限定（`:12` 元数据迁移从句、`:29` v0.3.7 性质注、`:1265` v0.3.7 自查行、`:1277` §18 v0.3.4 行、`:1280` §18 v0.3.7 行）。唯一不带版本限定的是 §17 的 **G1-b 行 `:1223`**，其原句是「而不是只把 ⟨旧锚点⟩ 换成新 hash」——它本身就在叙述「把它换掉」这个动作，语境即「被替换对象」。**未把这条判据偷偷放宽成「命中 0」**（那会把 G1-b 与 v0.3.4 / v0.3.7 三批的记录一起改掉），也**未改写** `:1223`：它是 v0.3.4 批次的自查行，按「历史行不改写」动它的代价大于收益。若 Leader 要它字面带限定，一句话我在该行尾加一枚限定指针、不改原句 |
| §6.3 那行是**连续子串的统一**、未夹带判据（授权边界原话：A 若连带改到别的现行断言即停下先报） | 按行首前缀把本版 §6.3 行与 `056ae71` 对应行配对（各须恰一行命中），做**整行等式断言** `旧行.replace(旧子串, 新子串) == 新行`——等式成立即等价于「除该子串外一字未动」，比词级 diff 强、且不需要人工数段落；再逐串断言该行携带的九项判据字面在位 | **等式成立**：差**恰一段**，就是那枚锚点连同它的状态短语与新增的同源指针。九项判据字面——「六个字段位」、`stepEvent`、「五条」归组与判定规则、`elapsedMs`、`exitCode`、`stdout`、`stderr` 的**非判据**、`stage = "EXTENSION"`、「契约是判据，词面是渲染」——新旧**逐字同在原位** ⇒ **停止条件未触发**，本版没有一处需要停下先报 |
| 新锚点下该行携带的**交叉引用不落空**（G1-b 立的规矩：换 hash 之外必须复核引用；不能只换 hash） | 取 `62b49a9:docs/ui/MERC-3/ui-spec.md` 实物，核 §6 章抬头与 §6.2 小节标题在位、主题仍是词面；取父提交元数据「平行输入」格末段那句 caveat 作核对串（由脚本从父提交提取，本行不复现其字面），数**本版新增文本**里该串的命中数，再单独断言本版 §6.3 行不含它 | §6 章抬头 =「文案矩阵（词面定稿）」、§6.2 =「阶段与步骤行」⇒「UI 词面归 ui-spec §6.2」在新锚点下**仍为真**，引用未落空。**caveat 未被复制第二份**：本版新增文本（20 个纯插入行 + 4 处追加片段 + §6.3 新行）里该串命中 **0**、§6.3 行命中 **0**；父提交侧它的三处既有命中（元数据「平行输入」格 + §17 G1-b 行 + §18 v0.3.4 行）里，**元数据那一格整行逐字未动** ⇒ 现行归属仍是且只是那一格，正是授权说的「别复制第二份」 |
| 元数据状态格是**纯加法**（授权 B 条原话：只做加法…不删旧句） | 整行**前缀包含**判据：本版 `:8` 须以「父提交 `:8` 去掉行尾收尾符」为前缀，且严格更长、且以表格行收尾符结束；再逐串断言 v0.3.5 / v0.3.6 / v0.3.7 三段的收官语句与两处过期措辞仍在行内原位 | **纯加法成立**：两处「待 Leader 通用门禁复跑」（v0.3.6 收尾处与 v0.3.7 收尾处）**一字未删**，本版只在其后追加现行状态句（G1 已通过 `01a0c540` + 本版为实现基线 + v0.3.7 / v0.3.8 为收口后文档同步批）⇒ 过期措辞按 Leader 评论 `01a0c56e` 说的「按下条一并处理」落地，处理方式就是这一句加法 |
| **判定文本零改动**（全量证明，不靠抽样） | 整文件多重集差：`Counter(父提交全部行) − Counter(本版全部行)` 与反向差，把「被改写的既有行」全集求出来再逐行分类；纯插入行不参与「改写」计数；另按 `## 1.` / `## 17.` 划出判定正文区间，数区间内的改动集合 | 被改写的既有行**恰 5 行**：元数据「版本」`:7`、「状态」`:8`、版本链 `:15`、§6.3 `:212`、§17 v0.3.7 表末行 `:1265`。其中 4 行是元数据与记账，**落在 §1–§16 判定正文的恰 1 行**（实测区间内改动集合 == `{212}`），而该行改的是锚点字面值、不触碰判据（上一行已证）⇒ **判据 / 口径 / 钉值的改动数 = 0**。父提交 1056 个正文行其余逐字保留；本版净增行**全部是纯插入**（§0 性质注 2 行 + 本自查块 + §18 本版 1 行）|
| 上一批与上两批的钉值零回退（不另列清单，由全量证明覆盖） | 上一行的「正文行逐字保留」已**穷尽**任何抽样：v0.3.5 钉的三处（§14.6 `stage` / §10 V-08 取值域 / §3.2 存在条件）、v0.3.6 落文的三行（§7.2 B-10 / §10 V-12 / 元数据「上游 PRD」）、v0.3.7 统一的两格（元数据「交付分支」/ §9.1）、**元数据「平行输入」格自身**（v0.3.6 起的 20 处未授权锚点之一）、KPI-02 口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、§18 历史行**必然在内**；另对 KPI-02 七段字面在 `§1..§16` 区间做命中数逐段比对 | **零回退**（由全量保留直接推出）；七段命中数新旧**逐段相等**（§17 / §18 排除在外，否则本节原文引用会自证污染）。特别说明本版**没有**触「平行输入」格——它已是同源基准，B 条只加状态格 |
| **尺子本身不空转** | 同一脚本跑两组对照：`--ref 056ae71`（本版父提交自身，未落文）与 `--ref 790ccd6`（隔一版） | 两组都 **exit 1**。父提交自身 FAIL 的判据逐一点名：① §1–§16 旧锚点归零（该侧仍命中 1 行 1 次）、② §6.3 整行等式（替换未发生）、③ 判定正文改动集合恰 `{212}`、④ 状态格纯加法、⑤ 本版新增文本零旧锚点命中不成立与否、⑥ 表数 +1、⑦ §18 本版行在位——全都不是父提交上成立的事实 ⇒ **PASS 不是判据太松**（逐项计数见随附输出） |
| 这一类滞后**自此收净**（不止修点名的那一处） | 全量扫本版 §1–§16 的 commit 形态字面（7 位十六进制）与 `@` 形态锚点，逐枚对读 `git ls-remote origin refs/heads/agent/{designer,architect,productmanager}/merc-3` 的现行头，判有无仍指陈旧值者 | §1–§16 内 commit 形态命中只剩 `590af8d`（上游 ADR，现行）与 `62b49a9`（UI，现行）两枚，余为评论号与 `1800000` / `1200000` 这类数字假阳性 ⇒ **除本行那枚外无第二枚陈旧锚点**，design 侧这一类滞后自此为零 |
| 表格结构与编号 | 剥 code-span 后统计列数的 lint（验法与 v0.3.3 起同一套，绝对数以本版实测为准）；对 `056ae71` 与本版各抽 `V-`/`B-`/`AC-`/`T-`/`F-`/`BR-`/`FR-`/`KPI-`/`OP-`/`D-P`/`D-N`/`N`/`G-`/`RISK-`/`SUG-`/`REV-`/`NT-`/`MF-`/`SG-`/`L-` 编号集合做对称差 | 表数 `71 → **72**`（本批 +1，即本自查表）、**列数不一致 0 张**、**缺分隔行 0 张**；编号**对称差为空** ⇒ 本版不引入任何新设计编号，新增标识符只有引用到的评论号 `01a0c56e` / `01a0c540` |
| 禁编造复扫（与 v0.2…v0.3.7 同口径） | 公网 URL 字面量、`x.y.z` 形态串集合、dnf-tw 真实版号片段各扫一遍并与 `056ae71` 做差；版本号字面量与「真实资料」不同源，比对前先按 `v\d+\.\d+\.\d+` 记法剥离 | URL **0**；`x.y.z` 集合与 `056ae71` **完全相同、新增 0、移除 0**；剥离后新增的版本号字面量只含 **`v0.3.8`** 自身 ⇒ 本批改的是一枚 **commit 锚点**，不引入任何真实版号 / URL / 目标路径 |
| 边界 | `git diff --name-only 056ae71 <本版>`；`git log --oneline 056ae71..<本版>`；`git merge-base --is-ancestor 056ae71 <本版>`；push 未用 `--force` / `--force-with-lease` | 仅命中本文件；未碰 `prd.md`、未碰 `docs/ui/MERC-3/*`、未碰功能代码；新 commit 续在 `056ae71` 之后**同一分支**（`agent/architect/merc-3`）、**恰一次提交**、**快进、不覆盖、不 force-push**；metadata-only ⇒ 按「对读基准版本」语义不触发反向回改，PRD 不回锚本版 |

**v0.3.9 自查（`applyVersionSelection` 签名对齐 + §16.3 防回潮指针批，全部可复跑；核对脚本与完整输出随回传评论附上）。本表 `:NNN` 一律指父提交 `3ea78ac`（v0.3.8）的行号**：

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| 三参签名**全量**收净（授权原话是「以 grep 收净为准」，不是「按点名清单改」） | 对父提交与本版各数 `applyVersionSelection(` 的全部命中，按「三参形态 / 两参形态 / 只提名不带括号」三形分类计数；另扫那枚被去掉的入参名的裸词命中（防入参名残留） | 父提交：`applyVersionSelection` 命中 **13 行**，其中**三参形态恰 2 行**（`:102` §3.3 组 D、`:778` §14.10 方案 B，正是授权点名的两处）→ 本版：三参形态 **0 行 0 次**、两参形态 **2 行**（同那两行）、其余 8 行「只提名、不带签名」的写法一字未动；那枚被去掉的入参名在父提交只出现在那两行内 → 本版全文 **0 次** ⇒ **无第三落点、无入参名残留**，本版记账亦不复现旧签名串（裸 grep 即干净，无须区分订正说明与现行断言） |
| 两处都是**单段替换**、夹带的是注释而不是判据 | 整行等式断言 `旧行.replace(旧子串, 新子串) == 新行`，旧子串取父提交该行的一段**连续片段**（含三参签名）——等式成立即等价于「除该段外一字未动」 | 两行等式**均成立** ⇒ `:102` 行其余部分（提交期 BR-07 撞键校验、清单 = PRD 原三项、`PLATFORM_IMAGE_TAG` 不进该清单、§14.4.2 引用、v0.3 订正标记、归属模块与「新增」列）与 `:778` 行其余部分（A / B 两案取舍叙述、三态各自行为、AC-22 判定改由服务层三态 + AC-07 重放承载、界面端到端前提不成立）**逐字在原位**；新增那一段的内容是「第三参由载荷 `configInfo.deployVersion` 取代」的签名说明与出处（§6.1 + §16.4 末段），不是新判据 |
| **判定正文改动集合恰那两行**（全量证明，不靠抽样） | 整文件多重集差 `Counter(父全部行) − Counter(本版全部行)` 求出「被改写的既有行」全集，再按 `## 1.` / `## 17.` 划出判定正文区间取交集比对 | 被改写的既有行**恰 5 行**：元数据「版本」`:7`、「状态」`:8`、版本链 `:15`、§3.3 组 D `:102`、§14.10 方案 B `:778`；前 3 行是记账，**落在 §1–§16 的改动集合 == {102, 778}**（机器实测集合相等）⇒ 除签名本体外**判据 / 口径 / 钉值的改动数 = 0**。父提交 1056 个正文行 ⇒ 本版正文 1058 行 = 1056 + 2（§16.3 指针段与其前空行），其余逐字保留；本版全部净增 **20 行均为纯插入**（四组：§0 性质注、§16.3 指针段、§17 本自查块、§18 本版行，各含其空行），无任何既有行丢失 |
| §16.3 那枚指针是**纯插入**，且判定正文不新增第二枚锚点 | ① N 表五行与「逐条校验…」段不参与改写计数（由上一行的全量集合相等覆盖）；② 全量扫两版 §1–§16 的 7 位十六进制字面（含数字假阳性），逐枚对读远端现行头 | ① `CatalogView` 签名块、四态表、N1…N5 五行、「逐条校验、任一不合规即整目录 `INVALID`」段**逐字未动**；② §1–§16 的 7hex 集合新旧**完全相同** = `590af8d`（上游 ADR，现行）/ `62b49a9`（UI，现行）/ `1800000`·`1200000`（`timeoutMs` 区间的数字假阳性）⇒ **本版没在判定正文里新设第二枚 PRD 锚点**，`prd.md` v0.7 的 commit 只出现在记账区（元数据状态格、本节、§18） |
| 上一批与上三批的钉值零回退（不另列清单） | 由「正文行逐字保留」的全量证明直接覆盖 | v0.3.5 三处（§14.6 `stage` 行 / §10 V-08 取值域 / §3.2 存在条件）、v0.3.6 三行（§7.2 B-10 / §10 V-12 / 元数据「上游 PRD」）、v0.3.7 两格、v0.3.8 的 §6.3 行、KPI-02 口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、元数据「平行输入」格、§18 历史行**必然在内** ⇒ 零回退 |
| **尺子本身不空转** | 同一脚本跑两组对照：`--ref 3ea78ac`（本版父提交自身，未落文）与 `--ref 790ccd6`（隔两版） | 两组都 **exit 1**：父提交自身 FAIL **18** 项、`790ccd6` FAIL **25** 项。逐项点名（父提交侧）：三参形态归零、两参形态在位、两处整行等式、判定正文改动集合恰 {102,778}、§16.3 指针在位、§3.3/§14.10 两处指针文本在位、版本格已升 v0.3.9、状态格纯加法、净增行全为纯插入、表数 +1、§18 本版行在位——**全都不是父提交上成立的事实** ⇒ PASS 不是判据太松 |
| 表格结构与编号 | 剥 code-span 后统计列数的 lint（验法与 v0.3.3 起同一套，绝对数以本版实测为准）；对两版各抽编号集合做对称差 | 表数 `72 → **73**`（本批 +1，即本自查表）、**列数不一致 0 张**、**缺分隔行 0 张**；编号**对称差为空**（新旧各 196 个）⇒ 本版不引入任何新设计编号。一处刻意的写法：§16.3 那枚指针**不复现 `prd.md` 侧那条风险编号的字面**（它落在本文的编号空间里会凭空多出一枚 `RISK-` 系标识符），改指「PRD §15 风险表对应行」，本行的「对称差为空」才成立 |
| 禁编造复扫（与 v0.2…v0.3.8 同口径） | 公网 URL、`x.y.z` 形态串（剥 code-span 后）、dnf-tw 真实版号片段各扫一遍并与父提交做差；版本号字面量比对前按 `v\d+\.\d+(\.\d+)?` 记法剥离 | URL **0**；`x.y.z` 集合与父提交**完全相同、新增 0、移除 0**；剥离后新增的版本号字面量只含 **`v0.3.9`** 自身与对 `prd.md` **v0.7** 的引用（后者是同批另一张批的现行值，不是拟定的未来值）⇒ 本批改的是一枚**方法签名的字面形态**，不引入任何真实版号 / URL / 目标路径 |
| 边界 | `git diff --name-only 3ea78ac <本版>`；`git log --oneline 3ea78ac..<本版>`；`git merge-base --is-ancestor 3ea78ac <本版>`；push 未用 `--force` / `--force-with-lease` | 仅命中本文件；未碰 `docs/ui/MERC-3/*`、未碰 `docs/design/adr/*` 与 `CONTEXT.md`（那是 release 分支上的 ⑥ 批）、未碰功能代码——**「实现不动、改文档」正是 Leader 裁定的裁定本体**；`prd.md` 的 v0.7 是**另一条分支上的另一张批**（`agent/productmanager/merc-3`），本版不代它落文；新 commit 续在 `3ea78ac` 之后**同一分支**、**恰一次提交**、**快进、不覆盖、不 force-push** |


**v0.3.10 自查（三处代码事实更正 + 两行原段内加限定，全部可复跑；核对脚本与完整输出随回传评论附上）。本表 `:NNN` 一律指父提交 `bf0681c`（v0.3.9）的行号**：本版在 §0 性质注处插入 2 行、§14.14 表内插入 1 行、§15.2 表内插入 1 行、本表插入 18 行、§18 插入 1 行 ⇒ `:319`→`:321`、`:882`→`:884`、`:946`→`:949`；脚本一律按行首形态锚定、不按行号锚定，偏移不影响任何一条判据。

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| **判定正文被改写的既有行恰三行**（§8.4 / §14.14 / §15.2），元数据三格另计 | 整文件多重集差 `Counter(父全部行) − Counter(本版全部行)` 求出被改写行全集，再逐行按行首形态归节 | 被改写既有行恰 6 行 = 元数据「版本」「状态」「版本链」3 + 上述 3；落在 §1–§16 的**恰 3 行** |
| **无第 9 处改动**（闭合式全量证明，不靠抽样） | 把本批 10 处授权替换按序施加到父提交文本，与本版全文逐字节比 | 逐字节相等 ⇒ 本版没有任何一处改动落在授权清单之外 |
| 三处都是**原段内单段替换**、不夹带判据 | 整行等式断言 `旧行.replace(旧段, 新段) == 新行`（三行各由自己那一段决定）；§14.14 另断言前缀 `String holder   = "EXT:" + instanceId + ":"` 逐字未动 | 三行各差恰一段；前缀未动 ⇒ 更正只落在后缀与注释 |
| 两枚新增行是**纯插入**、且在原表内、不新开判据 | ① §14.14 新行落在「与任务中心的边界」与「锁粒度」两行之间；② §15.2 新行落在「实现落点」行之后、`### 15.3` 之前；两表列数一致性另计 | 插入 2 行；两行的主语都是既有事实的取形与粒度，未新增任何一条判定 |
| 被更正的三处旧串**全文归零**（记账文本不复现旧串 ⇒ 裸 grep 即干净） | 对三个旧串各数本版与父提交命中 | 本版 0 / 0 / 0，父提交各 1 |
| **判定对象与核对物零改动**（12 行相邻判据逐字未动） | 按行首前缀锚定逐行比对两版：§10 V-26 holder 判据行、§14.14「与任务中心的边界」与「等待预算取 600 s」两行、§14.13.3「命令」行、§7.2 B-04 行、§8.2「每宿主机互斥」行、§4 `TaskMutexManager` 零改动行、§14.6 规则 4，以及本版**只报不改**的四处相邻滞后行（§1 `:47`、§15.1 首行、§7.2 B-13、§8.4 脚本注入行） | 12 行各 1→1 行、内容逐字相等 |
| 编号零改动 | 两版各抽 `V-/B-/AC-/T-/F-/BR-/FR-/RISK-/REV-/SUG-/OP-/KPI-/G-/D-P-/D-N-/L-/N1…N5` 集合做对称差 | 对称差为空（179 / 179） |
| 记账区不外泄到判定正文 | 全量扫两版 §1–§16 的 7–8 位十六进制形态字面与 `MERC-` 系 Issue 标识符 | 两集合各自完全相同（本版三行更正只引「文件:行号」，评论号只进 §0 / 本表 / §18） |
| 表格结构 | 剥 code-span 后按「以竖线开头的连续行成块」统计表数、列数一致性、缺分隔行 | 表数 73 → 74（本表）；列数不一致行 = 0；缺分隔行 = 0 |
| 禁编造复扫（与 v0.2…v0.3.9 同口径） | 公网 URL 字面量、`x.y.z` 形态串（剥 code-span 后）、真实版号片段各扫一遍并与父提交做差 | URL 0 / 0；`x.y.z` 集合不变；本版新增版本号字面量只含 `v0.3.10` |
| **尺子本身不空转**（反向对照） | 同一脚本 `--ref bf0681c`，把父提交当候选再跑一遍「本版新文本」断言 | 26 PASS / 12 FAIL / exit 1——「恰三行改写」「旧串归零」「新串到位」「无第 9 处改动」四组在旧 ref 上全部不成立 |
| 引用的代码事实**逐条对实物核**，不采信任何自述 | `carrier-v0310.py`：对 `SshUtil:356` / `:368`、`FileAccessService:254`、`TaskMutexManager:52` / `:68`、`DockerComposeAdapter:259-261`、`LinuxGsmDockerAdapter:222-223` 各取该行内容断言其形态；再对两处新落点的命令形状与清理粒度取交付分支实物断言 | 17 条全 PASS（代码事实与本文两版无关，故新旧两侧同判、不计入反向对照的 FAIL 数） |
| 边界 | `git diff --name-only bf0681c <本版>`；`git log --oneline bf0681c..<本版>`；`git merge-base --is-ancestor bf0681c <本版>`；push 未用 `--force` / `--force-with-lease` | 仅命中 `docs/design/MERC-3/design.md`；恰一个提交；新 commit 续在 `bf0681c` 之后**同一分支**（`agent/architect/merc-3`）⇒ 快进、不覆盖、不 force-push；未碰 `prd.md`、`docs/ui/MERC-3/*`、功能代码 |

**v0.3.11 自查（四处同类相邻滞后的机械更正，零纯插入行；核对脚本与完整输出随回传评论附上）。本表 `:NNN` 一律指父提交 `f3f3703`（v0.3.10）的行号**：v0.3.10 那批登记四处滞后时写的是它自己的父提交 `bf0681c` 的行号（§1 `:47`），本版父提交上该句已随 §0 性质注的 2 行插入移到 `:49` ⇒ 本表按父提交实测行号给：§1 `:49`、§15.1 首行 `:932`、§7.2 B-13 `:245`、§8.4 `:318`；本批净插入 22 行（全文 1344 → 1366 行）= §0 性质注 2 + 本自查块 19（表 16 行 + 引言行与空行 3）+ §18 行 1；正文四行原地改写、不移动任何行；脚本一律按行首形态锚定、不按行号锚定，偏移不影响任何一条判据。

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| **判定正文被改写的既有行恰四行**（§1 / §15.1 / §7.2 B-13 / §8.4），元数据三格另计 | 整文件多重集差 `Counter(父全部行) − Counter(本版全部行)` 求出被改写行全集，再逐行按行首形态归节 | 被改写既有行恰 7 行 = 元数据「版本」「状态」「版本链」3 + 上述 4；落在 §1–§16 的**恰 4 行** |
| **无第 11 处改动**（闭合式全量证明，不靠抽样） | 把本批 10 处授权替换按序施加到父提交文本，与本版全文逐字节比 | 逐字节相等 ⇒ 本版没有任何一处改动落在授权清单之外 |
| 四处都是**原段内单段替换**、不夹带判据 | 整行等式断言 `旧行.replace(旧段, 新段) == 新行`（四行各由自己那一段决定）；B-13 行另断言核对物列逐字未动、§15.1 行另断言第 2 列 `30 s` 与第 3 列的 `FileAccessService.java:264` 逐字未动 | 四行各差恰一段；核对物与取值出处未动 ⇒ 更正只落在读法标签与命令形状 |
| 零新增判定行、零纯插入行 | §1–§16 判定正文行数两版相同（1061 → 1061）；全文 1344 → 1366 行的差额恰为记账区净插入 22 行（§0 性质注 2 + 本自查块 19 + §18 行 1） | §1–§16 无一行新增、无一行被删 ⇒ 未新开任何判据 |
| 被更正的四处旧串**全文归零**（记账文本不复现旧串 ⇒ 裸 grep 即干净） | 对四个旧片段各数本版与父提交命中 | 本版 0 / 0 / 0 / 0，父提交各 1 |
| **正文内不再有裸命令形状**（本批的收口判据） | 数 §1–§16 里「bash <file>」与带前缀形态各自的命中 | `bash <file>` 3 次、`timeout <秒> bash <file>` 3 次 ⇒ 三处形状全部带护栏前缀；父提交为 3 与 1 |
| **判定对象与核对物零改动**（10 行相邻判据逐字未动） | 按行首前缀锚定逐行比对两版：§10 V-26 holder 判据行、§15.2「实现落点」与「秒粒度」两行（v0.3.10 刚落、本批不得再动）、§15.2「缺省值」行（本版只报不改）、§15.3 首行与 §12 RISK-D06 行（同报不改）、§14.13.3「命令」行、§14.6 规则 4、§8.4「命令面扩大」行、§14.14「holder 后缀的实际取形」行 | 10 行各 1→1 行、内容逐字相等 |
| 编号零改动 | 两版各抽 `V-/B-/AC-/T-/F-/BR-/FR-/RISK-/REV-/SUG-/OP-/KPI-/G-/D-P-/D-N-/L-/N1…N5` 集合做对称差 | 对称差为空（179 / 179） |
| 记账区不外泄到判定正文 | 全量扫两版 §1–§16 的 7–8 位十六进制形态字面与 `MERC-` 系 Issue 标识符 | 两集合各自完全相同（本版四行更正只引「文件:行号」，评论号只进 §0 / 元数据 / 本表 / §18） |
| 表格结构 | 剥 code-span 后按「以竖线开头的连续行成块」统计表数、列数一致性、缺分隔行 | 表数 74 → 75（本表）；列数不一致行 = 0；缺分隔行 = 0 |
| 禁编造复扫（与 v0.2…v0.3.10 同口径） | 公网 URL 字面量、`x.y.z` 形态串（剥 code-span 后）、真实版号片段各扫一遍并与父提交做差 | URL 0 / 0；`x.y.z` 集合不变；本版新增版本号字面量只含 `v0.3.11` |
| **尺子本身不空转**（反向对照） | 同一脚本 `--ref f3f3703`，把父提交当候选再跑一遍「本版新文本」断言 | 29 PASS / 19 FAIL / exit 1——「恰四行改写」「四处旧串全文归零」「正文新串精确计数」「正文内不再有裸命令形状」「无第 11 处改动」「原段内单段替换等式」六组共 19 条断言，在旧 ref 上全部不成立 ⇒ 尺子不空转 |
| 引用的代码事实**逐条对实物核**，不采信任何自述 | `carrier-v0311.py`：对 `FileAccessService:254` / `:263` / `:264`（两参重载缺省 `30_000L`）、`SshUtil:356`（`timeoutMs` 流向 `getOrCreateSession`）→ `:135` → `:152`（该值只喂 `connect`）/ `:368`（`executeRemoteCommand` 无超时实参，另断言该行不含任何 timeout 字面）、`DockerComposeAdapter:257`+`:260` 与 `LinuxGsmDockerAdapter:221`+`:223`（shell `timeout` 同形先例 + 「仅作用于建连」现场注释）、交付头 `ExtensionScriptRunner:96`（`timeout <秒> bash <file>` 实测形状）/ `:34`（代码侧自述「而不是裸」）/ `:175`（向上取整）/ `:103`（按远端被杀时刻识别）/ `:30`（SIGTERM 不保证已终止），以及 §15.1 表内本批未动的相邻锚点（`PatchInstallExecutor:54`、`DeployTaskHandler:50`、两处 60 s 硬编码）各取该行内容断言其形态 | 22 条全 PASS × main / release / 交付头（代码事实与本文两版无关，故新旧两侧同判、不计入反向对照的 FAIL 数）。**顺带核出的一条引注口径**：本文的代码行号一律钉在 main 上——release 头并入 MERC-20 后 `DockerComposeAdapter` 的 compose-exec 那行由 `:728` 下移到 `:781`（同一语句、行号漂移），故该行号只在 main 侧断言、并在 release / 交付头同判其新落点。本批四行更正所引的行号（`SshUtil:356` / `:368`、`FileAccessService:254` / `:264`、`DockerComposeAdapter:257`+`:260`、`LinuxGsmDockerAdapter:221`+`:223`）三个 ref 上同判、未漂 ⇒ 漂移只发生在本文既有的另一枚引注上（本批未动那一行），不是本批引入的问题，登记交 Leader 判是否另批处置 |
| 边界 | `git diff --name-only f3f3703 <本版>`；`git log --oneline f3f3703..<本版>`；`git merge-base --is-ancestor f3f3703 <本版>`；push 未用 `--force` / `--force-with-lease` | 仅命中 `docs/design/MERC-3/design.md`；恰一个提交；新 commit 续在 `f3f3703` 之后**同一分支**（`agent/architect/merc-3`）⇒ 快进、不覆盖、不 force-push；未碰 `prd.md`、`docs/ui/MERC-3/*`、功能代码 |

**v0.3.12 自查（同族表述三处的归因更正，正文零纯插入行；核对脚本与完整输出随回传评论附上）。本表 `:NNN` 一律指父提交 `e1efc7a`（v0.3.11）的行号**：§15.2「缺省值」`:945`、§15.3 首行 `:956`、§12 RISK-D06 行 `:452`（与 Leader 派单评论 `01a0d484` §五-1 ① 所写行号逐字相同，本版实测一致）；本批净插入 24 行（全文 1366 → 1390 行，split 口径 = wc -l 再 + 1）= §0 性质注 2 + 本自查块 21 + §18 行 1；正文三行原地改写、不移动任何行；脚本一律按行首形态锚定、不按行号锚定，偏移不影响任何一条判据。

| 自查项 | 方法 | 结果 |
| --- | --- | --- |
| **判定正文被改写的既有行恰三行**（§15.2 缺省值 / §15.3 首行 / §12 RISK-D06），元数据三格另计 | 整文件多重集差 `Counter(父全部行) − Counter(本版全部行)` 求出被改写行全集，再逐行按行首形态归节 | 被改写既有行恰 6 行 = 元数据「版本」「状态」「版本链」3 + 上述 3；落在 §1–§16 的**恰 3 行** |
| **无第 10 处改动**（闭合式全量证明，不靠抽样） | 把本批 9 处授权替换按序施加到父提交文本，与本版全文逐字节比 | 逐字节相等 ⇒ 本版没有任何一处改动落在授权清单之外 |
| 三处都是**原段内单段替换**、不夹带判据 | 整行等式断言 `旧行.replace(旧段, 新段) == 新行`（三行各由自己那一段决定）；§15.2 缺省值行另断言第 2 列 `**`600_000 ms`（10 分钟）**` 与推导结论末句逐字未动、RISK-D06 行另断言第 1/3/4 格逐字未动 | 三行各差恰一段；取值与缓解措施未动 ⇒ 更正只落在归因 |
| **结论与登记对象零改动**（同批不许动的东西逐字断言） | 断言 §15.3 的 `**不保证远端进程已被终止**` 与引导词「因此：」及三条子弹、「登记为 RISK-D06」所在行、§15.2 其余五行、§15.1 首行（v0.3.11 刚落）、§1 引导句与 §7.2 B-13 与 §8.4 脚本注入行（v0.3.11 刚落）、§14.13.3「命令」行、§10 V-22 行、相邻 RISK-D05/D07/D08 行在本版逐字未动 | 21 行各 1→1 行、内容逐字相等 |
| 零新增判定行、正文零纯插入行 | §1–§16 判定正文行数两版相同（1061 → 1061）；全文 1366 → 1390 行的差额恰为记账区净插入 24 行 | §1–§16 无一行新增、无一行被删 ⇒ 未新开任何判据 |
| 被更正的三处旧串**在 §1–§16 归零**（记账区的历史句按「历史句不改写」口径原样保留 ⇒ 归零断言按正文范围做） | 对三个旧片段分别数父与本版的全文件命中与 §1–§16 命中 | 正文 0 / 0 / 0（父各 1）；那枚旧括注在全文件的命中 4 → 3，减少的恰是 §15.2 那一处，其余三处在元数据「状态」格、§0 v0.3.11 性质注、§18 v0.3.11 行里原样未动 |
| **同族表述自此收净**（本批的收口判据） | 数 §1–§16 里「不再等待」的命中，并断言每一处所在行同时含远端 `timeout` 的指名 | 「不再等待」2 次、两处所在行都含远端 `timeout` ⇒ 正文再无「把不等待归给通道入参」的读法；同族标记 `通道默认` 在正文命中 0 |
| 相邻两行经核对**不属本族**、逐字未动（不为改而改） | §15.2「超时后的判定」行只引用 ui-spec 的既有词面（不含机制归因）、§14.13.3「命令」行本就写着 shell `timeout 1200` | 两行各 1→1 行、逐字相等；已作为判据写进上行 |
| 编号零改动 | 两版各抽 `V-/B-/AC-/T-/F-/BR-/FR-/RISK-/REV-/SUG-/OP-/KPI-/G-/D-P-/D-N-/L-/N1…N5` 集合做对称差 | 对称差为空（179 / 179） |
| 记账区不外泄到判定正文 | 全量扫两版 §1–§16 的 7–8 位十六进制形态字面与 `MERC-` 系 Issue 标识符 | 两集合各自完全相同（本版三行更正只引「文件:行号」，评论号只进 §0 / 元数据 / 本表 / §18） |
| 表格结构 | 剥 code-span 后按「以竖线开头的连续行成块」统计表数、列数一致性、缺分隔行 | 表数 75 → 76（本表）；列数不一致行 = 0；缺分隔行 = 0 |
| 禁编造复扫（与 v0.2…v0.3.11 同口径） | 公网 URL 字面量、`x.y.z` 形态串（剥 code-span 后）、真实版号片段各扫一遍并与父提交做差 | URL 0 / 0；`x.y.z` 集合不变；本版新增版本号字面量只含 `v0.3.12` |
| **尺子本身不空转**（反向对照） | 同一脚本 `--ref e1efc7a`，把父提交当候选再跑一遍「本版新文本」断言 | 实测 44 PASS / 21 FAIL / exit 1（完整输出随回传评论附上）——「恰六行改写且配对到锚点」「改写行身份锚点唯一」「正文三处旧串归零」「三处旧串全文件归零」「记账区历史句仍在」「正文新串精确计数（9 串）」「同族表述自此收净」「无第 10 处改动」「原段内单段替换等式（三行）」七组共 21 条断言在旧 ref 上全部不成立 ⇒ 尺子不空转 |
| 引用的代码事实**逐条对实物核**，不采信任何自述 | `carrier-v0312.py`：对本批三行更正所依据的每一条事实取该行内容断言其形态——`SshUtil:356`（`timeoutMs` 流向 `getOrCreateSession`）/ `:368`（`executeRemoteCommand` 无超时实参，另断言该行不含任何 timeout 字面）、`FileAccessService:254` / `:263` / `:264`（两参重载缺省 `30_000L`）、交付头 `ExtensionScriptRunner` 类注释 :29-31（自述「不再等待 / 不保证已终止 / SIGTERM 可 trap / 子进程可存活」四条）/ `:61`（`SHELL_TIMEOUT_EXIT_CODE = 124`）/ `:96`（远端 `timeout` 形状）/ `:103`（退出码 124 按远端被杀时刻识别）/ `:175`（向上取整 + 下界 1 s）、以及 §15.2「缺省值」行推导所依赖的 `PatchInstallExecutor:54`（600 s）与 §15.1 的 30 s 出处 | 22 条全 PASS × main / release / 交付头 |
| **引注 main 侧逐条对账**（Leader 评论 `01a0d484` §二 裁定的口径：行号钉 main、漂移只登记） | `anchor-audit-v0312.py`：从本版 §1–§16 抽出全部「文件名:行号」形态的代码引注，逐条回 main `3c5df3e` 实物核该行存在且含预期语义标记；再对 release 头 / MERC-20 交付头做同一批引注的位置核对，只登记漂移 | 自动核对 124 条（引注自带类名 67 + 裸 `:NNN` 由「前窗唯一线索」或「同行唯一类名」回指者 57）：**main 命中 121 / 标出 3**；标出的三条经人工回看实物的定性 = ① §14.13.2「调用方」行的 `DeployService:163` 落在空行、真身在 `:164`（`adapterFactory.getAdapter(deployType)`）；② §14.14「等待预算取 600 s」行的 `PatchInstallHandler:33` 落在空行、真身在 `:32`（`DEFAULT_TIMEOUT_MS = 60 * 60 * 1000L`）——两枚都是本文既有的行号不精确，**不在本批授权面、只报不改**，交 Leader 派后续小批；③ `:237-255` 是核对器把「同上 `:237-255`」回指成同行唯一类名 `GameVO` 所致，实物在 `GameServiceImpl.java:237-255`（该行段确有 `supportedDeployTypes` 合并码）⇒ 属核对器的回指限制、不是文档错。另有裸 `:NNN` 90 条线索不唯一 ⇒ 不猜、逐条列人工回指清单；release / 交付头漂移 80 处读数（同一语句换行、事实未变）按 Leader §二 只登记。**同一把尺子跑父提交 `e1efc7a` 得 124 / 121 / 3 / 80 / 90 完全相同的读数 ⇒ 本批未引入、也未消除任何一行号引注**（§15.3 新增的那枚实物指针故意只给类名）|
| 边界 | `git diff --name-only e1efc7a <本版>`；`git log --oneline e1efc7a..<本版>`；`git merge-base --is-ancestor e1efc7a <本版>`；push 未用 `--force` / `--force-with-lease` | 仅命中 `docs/design/MERC-3/design.md`；恰一个提交；新 commit 续在 `e1efc7a` 之后**同一分支**（`agent/architect/merc-3`）⇒ 快进、不覆盖、不 force-push；未碰 `prd.md`、`docs/ui/MERC-3/*`、功能代码 |
...
| v0.3.5 | 2026-09-22 | Spec（按 Leader 裁定评论 `01a0c4d8` 落文，非 @Architect 新判定） | **SUG-21 的裁定落文批**：v0.3.4 把「`stepId == null` 两类条件行钉什么 `stage`」登记为待裁定，Leader 裁定**采用本设计提出的拆分**（不选单支、不挂实现期），裁定依据正是 v0.3.4 给的两组 UI 实物——统一取 `EXTENSION` 被 `LG`（带 × + 进入行 ×）证伪、统一取既有阶段被 `LE`/`LF`（带 √ + 进入行 √）证伪 ⇒ 拆分是唯一与实物、§14.11 latch、§16.4 `INVALID` 同时自洽的写法。**单独一批、单独 commit**（「三件同批」在 ③ 上做不到 ⇒ 零轮次记账延续、不占评审轮次），判定文本恰动三处：**① §14.6 `stage` 行**分别钉值——BR-12 拦截行 = `"EXTENSION"`（阶段以「入口判定不合法」短暂成立，产进入行 + 拦截行即 `ERROR` 终止），目录不合法说明行 = **它实际发生所在的既有阶段**（本期该点在 `DEPLOY` 完成行后、段不成立 ⇒ `"DEPLOY"`；**钉的是判据不是字面量**，实现前移读目录则随之而移）；**② §10 V-08 首判据**取值域写明「**扩展阶段内的行**」而非 logs 全部行（旧字面有两种读法，按全部行读会把正确实现判成违规），两侧仍只读字段：`stepId != null` 的步骤行必带该常量、`stepId == null` 的阶段级行按 ① 归属，BR-12 拦截行在域内、目录不合法说明行在域外；**③ §3.2 `:81` 存在条件两支化**（裁定书面授权的同批对齐——同一缺陷拆开修＝各修一半）：无步骤集但入口判定不合法须 BR-12 拦截 ⇒ 段短暂成立。**未动**：KPI-02 的分子与分母口径七段逐字保留（两行 `stepId == null`，取何值都不进分母，与 SUG-13 的可满足性论证无涉）、§14.6 `stepId` 行七项枚举与「定死」措辞、规则 2、`stepEvent`/`elapsedMs` 两行、§16.4 `INVALID`/`EMPTY` 两行、§14.11 小节标题与 P3 latch 行、§3.2 `:82` 段内序列与 `:83` 终止条件、§18 三条历史行、元数据三行——**共 22 处逐字未动**（机器核对见 §17 v0.3.5 自查表，含「同一把尺子跑 v0.3.4 必须 FAIL」的反向对照）。**超授权面的处理**：依赖该存在条件的相邻五行逐行核对，三行（F-02 / V-10 / §14.7 `:712`）主语是「无扩展声明的游戏」、与 BR-12 触发条件互斥 ⇒ 不矛盾、未动；**B-10 `:230` 与 V-12 `:373` 两处的字面（「无步骤路径」「无扩展步骤：整条序列不含 `EXTENSION`」）在本裁定下能罩到 BR-12 那一路 ⇒ 按 `01a0c4d8` 第 2 条「超出『EXTENSION 段的存在条件』这一句即停下先报」一字未改，连 ≤20 字的最小改法一并回报 Leader**。另登记一处已知滞后：本文件「上游 PRD」锚点仍指 `270f9d0`（v0.5），PRD v0.6 回写批在本版之后成 commit，两文件互为锚点属循环，故该锚点的更新留作下一次自然触手（已在回传评论点名）。新 commit 续在 `c8a6f3f` 之后，不覆盖、不 force-push；`prd.md` 与 `docs/ui/MERC-3/*` 零改动、功能代码零改动。禁编造复扫 URL 命中 0、`x.y.z` 集合与 v0.3.4 相同且新增 0 |
| v0.3.6 | 2026-09-22 | Spec（按 Leader 裁定评论 `01a0c512` 第 ① ② 条落文，非 @Architect 新判定） | **授权补项批 + 元数据锚点统一批（合一版单次提交，零轮次记账延续、不占评审轮次）**。① v0.3.5 按「超出『EXTENSION 段的存在条件』这一句即停下先报」的一格缝，Leader 判「补」（理由与 v0.3.5 自给的最小改法同源：不补就是把这格留给 G1 之后 @Tester 的脚本），本版按该最小改法落文：**§7.2 B-10「无步骤路径」→「无扩展声明游戏的路径」**、**§10 V-12「无扩展步骤」→「无扩展声明（未触发 BR-12）」**——词级各只动一段（核对见 §17 v0.3.6 自查表第二行），两行要判的东西与追溯列一字未变，判据对象仍是 AC-15 那批无扩展声明的游戏；§7.3 F-02 / §10 V-10 / §14.7 三行按 v0.3.5 的结论照旧未动。② 元数据「上游 PRD」锚点自 `270f9d0`（v0.5）回改至 **`ee86825`（v0.6 技术设计回写批）**，并在该格把锚点语义钉死为「**对读基准版本**」：两文件互为锚点属循环 ⇒ metadata-only 升版不触发反向回改，PRD 侧不必回锚本版、本设计亦不代其回改（`prd.md` 本版零改动），两文件自此不互相追版本；v0.3.5 登记的「已知滞后」在此闭合。**本版判定文本恰动三行**；v0.3.5 未授权锚点 22 项中除授权转出的三行外**其余 19 处 + §18 v0.3.5 历史行 = 20 处逐字未动**，v0.3.5 刚钉下的三处（§14.6 `stage` 行 / §10 V-08 首判据取值域 / §3.2 EXTENSION 段存在条件）与 KPI-02 的七段口径**零回退**；那三行在核对里由「未授权锚点（逐字未动）」**转为授权改动（逐字断言新文本）**，口径与 `01a0c512` 一致。边界复核：改动超出这三行即停下先报——本版未触发（§17 登记表末行只加一枚授权指针、按「历史行不改写」原样保留该行的历史表述）。新 commit 续在 `347faf3` 之后，不覆盖、不 force-push；`prd.md` 与 `docs/ui/MERC-3/*` 零改动、功能代码零改动。机器核对：`selfcheck-design-v036.py --old 347faf3` 本版 PASS（组 (1)–(6)），同一把尺子跑未落文基准 FAIL 13 项、跑上一批 FAIL 19 项；禁编造复扫 URL 命中 0、`x.y.z` 集合与 v0.3.5 相同且新增 0 |
| v0.3.7 | 2026-09-22 | Spec（按 Leader 派工评论 `01a0c552` 落文，非 @Architect 新判定） | **交付分支名统一批（metadata-only、零轮次记账延续、不占评审轮次）**：本 Issue 的交付分支由 Leader 在 G1 汇总（评论 `01a0c526`）声明为 `release/MERC-8-extension-steps-dnf-tw`，而本文两处仍写 MERC-3 时期拟定的旧名——元数据「交付分支」格、§9.1「分支与 ADR 载体」行；拆票回传（评论 `01a0c552`）把它点名为「声明的分支名与实现票面唯一的不一致处」，Leader 同评论授权本批并**把范围锁死为分支名替换**：全部出现处一次 grep 收干净、凡需改语义处停下先报、单独 commit、只触本文件、禁 force-push。落文：上述两格的分支名各统一为新名。实测被替换的旧值在 `790ccd6` 出现 **恰 2 处**（正是这两格）、**本版全文命中 0**；第二形态（简写 / 去前缀裸名 / 英文写法）各 **0**，ADR 那个文件名的字面新旧相等、未被牵连 ⇒ 授权说的「若另有落点一并」**没有另有落点**。新名在本文的位置 = 那两格 + 本版记账引用，**落在 §1–§16 判定正文的恰 §9.1 一行**。两行均由**整行等式** `旧行.replace(旧名, 新名) == 新行` 证明是纯值替换 ⇒ §9.1 该行携带的另一件事（ADR-0029 与 `CONTEXT.md` / `glossary.md` 须从 `agent/leader/chat-1cfa252af963 @ 590af8d` **merge 带入**）一字未动、在新名下复核仍为真 ⇒ **「需改语义即停下先报」的停止条件未触发**。写法上另取一条自律：**本版全部记账文本不复现被替换的旧值字面串**（只以「MERC-3 时期拟定的旧名」指代），故本批不像 v0.3.6 那样需要靠「旧字面只作订正说明、不作现行断言」来判——裸 grep 即干净。**判定文本恰零改动**：整文件多重集差给出被改写的既有行**恰 5 行**（元数据「版本」/「状态」/「交付分支」三格 + 版本链一句 + §9.1 那一行），前四行全是记账，落在正文的恰 1 行且改的是字面值而非判据；旧行被删除的出现次数**恰 5**（= 这 5 行的旧版本，除此之外无任何既有行丢失），全文净增 **17 行且全是纯插入**（§0 性质注 2 行 + §17 自查块 + §18 本版一行）⇒ `790ccd6` 的 1263 个正文行中 **1258 行逐字保留**。这是**全量证明**而非抽样，故 v0.3.5 钉的三处、v0.3.6 落文的三行、KPI-02 的分子分母口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、§18 历史行、元数据「上游 ADR」「平行输入」两行**零回退**由它直接覆盖；另对 KPI-02 七段字面在 §1..§16 区间做命中数逐段比对，新旧**逐段相等**。**本批不新增任何事实**：不引入真实版号 / URL / 目标路径（禁编造复扫 URL 命中 0、`x.y.z` 集合与 `790ccd6` 相同且新增 0）、不引入新设计编号（编号对称差为空）。metadata-only 升版按 v0.3.6 在「上游 PRD」格钉死的「**对读基准版本**」语义**不触发任何反向回改** ⇒ `prd.md` 与 `docs/ui/MERC-3/*` 零改动、功能代码零改动；文档与 ADR 的目录名 `docs/*/MERC-3/` 按 MERC-8 的继承约定**不重命名**（避免与已 push 产物的路径错位），故本版之后「目录写 MERC-3、交付分支写 MERC-8」这一形态仍成立，但**分支名自此无处是旧的**。另登记一处相邻滞后**未改**（不是分支名、改它须连带判一句 UI 定稿状态的现行真假 ⇒ 超出授权面，停下先报）：§6.3 的「UI 词面归 ui-spec §6.2」那句仍指 `f6312ef`（UI v0.1），与元数据「平行输入」格已在 G1-b 统一到的 `62b49a9`（v0.6）**不同源**，交 Leader 定或留作下一次自然触手。机器核对：`verify-v037.py --old 790ccd6 --new <本版>` 全 PASS，含「同一把尺子跑 `--ref 790ccd6`（未落文基准）必须 FAIL」的反向对照；表数 `70 → 71`、列数不一致 0、缺分隔行 0。新 commit 续在 `790ccd6` 之后、恰一次提交、不覆盖、不 force-push |
| v0.3.8 | 2026-09-22 | Spec（按 Leader 授权评论 `01a0c56e` 落文，非 @Architect 新判定） | **§6.3 UI 锚点同源批（metadata-only、零轮次记账延续、不占评审轮次）**：v0.3.7 扫锚点时登记「§6.3 仍指 UI v0.1 那枚旧锚点、与元数据「平行输入」格的 `62b49a9` 不同源」并按边界停下先报；本版 Leader 补齐依据并授权单开一批（评论 `01a0c56e`）⇒ 依据两条都在本文一侧可核：**元数据「平行输入」格自身**已把 `62b49a9` 读作「v0.6 / S1b 定稿」（v0.3.4 的 G1-b 复核落文），且 **G1 已由人类通过**（评论 `01a0c540`），故 §6.3 里那句「S1b v0.4 在途」不再是可留在正文的现行断言。落文恰两条、范围锁死：**A** §6.3 那枚锚点做**一个连续子串**的统一（旧值 → `62b49a9`，状态短语 →「v0.6 / S1b 定稿」，同处加一枚**指向元数据「平行输入」格的并收状态指针**），使那句「UI 侧并收仍挂账」的 caveat 全文只留那一格、正文不复制第二份；换锚点前按 G1-b 的规矩**复核该行携带的交叉引用**在新锚点下不落空（`ui-spec @ 62b49a9` 的 §6 抬头 =「文案矩阵（词面定稿）」、§6.2 =「阶段与步骤行」）⇒ 引用仍为真。**B** 元数据状态格**只做加法**：追加一句现行状态（G1 已通过 `01a0c540`、本版为实现基线、v0.3.7 / v0.3.8 为收口后文档同步批），上方各版旧句与两处过期措辞一字未删。**停止条件未触发**：§6.3 那行由**整行等式** `旧行.replace(旧子串, 新子串) == 新行` 证明差恰一段，该行九项判据字面（六个字段位 / `stepEvent` / 五条规则 / `elapsedMs` / `exitCode` / `stdout` / `stderr` 非判据 / `stage` 常量 / 契约与词面关系）逐字在原位 ⇒ 未连带改到别的现行断言。**判定文本零改动**：整文件多重集差给出被改写的既有行**恰 5 行**（元数据「版本」与「状态」两格、版本链一句、§6.3 那行、§17 那行 v0.3.7 表末行的授权指针），落在 §1–§16 判定正文的恰 1 行且改的是锚点字面值；父提交 1056 个正文行其余逐字保留，本版净增行全为纯插入 ⇒ 这是**全量证明**而非抽样，v0.3.5 三处、v0.3.6 三行、v0.3.7 两格、**元数据「平行输入」格自身（本版未触它）**、KPI-02 口径、§14.6 `stepId` 行七项枚举、§16.4 四态表、§14.11 latch 行、§18 历史行零回退由它直接覆盖。**类别收净**：全量扫本版 §1–§16 的 commit 形态锚点，除被改的这枚外**无第二枚陈旧锚点**，本文对 UI 的两枚锚点自此同源。写法沿用 v0.3.7 已获认可的自律：**本版新写文本不复现旧锚点字面串**，全文剩余的旧锚点命中全部是既有历史行（本版新增 0 处）；其中 5 行带 `（v0.1）`/`（UI v0.1）` 版本限定，§17 G1-b 行 `:1223` 靠「把它换成新 hash」的语境自证为被替换对象，按「历史行不改写」未动。**未授权扩展**：不改元数据「平行输入」格、不动 `prd.md` 与 `docs/ui/MERC-3/*`、不替 UI 侧做那笔并收（本小队无 UI 文档 owner，Leader 判为不开空票、已并入 G3 汇总的已知滞后）。metadata-only 升版按「对读基准版本」语义不触发反向回改 ⇒ `prd.md`、`docs/ui/MERC-3/*`、功能代码零改动。机器核对：`verify-v038.py --old 056ae71 --new <本版>` 全 PASS，含「同一把尺子跑 `--ref 056ae71`（未落文基准）必须 FAIL」的反向对照；表数 `71 → 72`、列数不一致 0、缺分隔行 0、URL 0、`x.y.z` 集合与 `056ae71` 相同且新增 0。新 commit 续在 `056ae71` 之后、恰一次提交、不覆盖、不 force-push |
| v0.3.9 | 2026-09-24 | Spec（按 Leader 裁定评论 `01a0cf20` §四 落文，非 @Architect 新判定） | **`applyVersionSelection` 签名对齐 + §16.3 防回潮指针批（零轮次记账延续、不占评审轮次）**：MERC-18 的交付回传（评论 `01a0cf18` §二）把「本文 §14.10 表格里的三参签名比实现多一个入参」列为已知限制，交 Leader 裁定。Leader 判「**取 2 参、实现不动、改文档**」（评论 `01a0cf20` §四），三条依据全在本文一侧：① §6.1 的 `POST` 格已写「无新增字段：版本以 `configInfo.deployVersion` 承载」；② §16.4 末段已把「写 / 删 / 不写」的决定钉在提交载荷上；③ 独立的选择入参与载荷同源 ⇒ 同一事实两份真相、谁先生效无从判定。并指名「§14.10 那处签名在方案对比表内，不是签名契约」⇒ 改它不触及任何判据。<br>**落文恰两件事**：(1) `:102`（§3.3 组 D）与 `:778`（§14.10 方案 B）两处签名由三参统一为两参，并在同一处注明第三参由 `configInfo.deployVersion` 取代（出处指 §6.1 + §16.4 末段）——grep 收净结果：父提交三参形态**恰这 2 行**，本版**归零**，两参形态**恰这 2 行**，另 11 处「只提名不带签名」的写法一字未动，那枚被去掉的入参名本版全文命中归零 ⇒ **无第三落点、无入参名残留**。(2) §16.3 的 N1…N5 清单旁加一枚**防回潮指针**（纯插入）：撞键校验只在提交期、N1…N5 刻意不含「`deployVersion` 与 `variables[].name` 同名 ⇒ 目录不合法」，并写明「要动这五条的项数须回头重跑 G1」；它的对立面（补第 6 条声明期规则）由 `prd.md` 同批的 v0.7 关掉，两处同源、互为指针。<br>**判定文本零改动（除那两行签名本体）**：整文件多重集差给出被改写的既有行**恰 5 行**（元数据「版本」/「状态」/「版本链」三处记账 + §3.3 组 D + §14.10 方案 B），落在 §1–§16 判定正文的改动集合 == {102, 778}；其余净增行全为纯插入 ⇒ 这是**全量证明**而非抽样，v0.3.5 三处、v0.3.6 三行、v0.3.7 两格、v0.3.8 的 §6.3 行、§16.3 的 N 表五行与四态表、KPI-02 口径、§14.6 `stepId` 行七项枚举、§14.11 latch 行、§18 历史行零回退由它直接覆盖。<br>**不回改「上游 PRD」锚点格**：本版两处改动不依赖 PRD 的任何新增文本，按 v0.3.6 在该格钉死的「**对读基准版本**」语义，metadata 与判定正文均不新设第二枚 PRD 锚点（v0.7 的 commit 只在记账区出现，§1–§16 的 7hex 集合新旧完全相同）；两文件不互相追版本。<br>**④ 的裁定一并落在此处记账**：BR-07 撞键清单只认表快照、不按 `getDeployConfigs()` 合并视图校验（RISK-D07 那条通道缺陷）判「本期不另立 Issue」——§14.4.1 R2 已把该通道显式排除在本期（不修、不测），无任何 AC 依赖它，且校验对象与部署组装对象**同源是对的**（换成合并视图才会让两者脱钩）；处理同 OP-07 口径，进 G3「已知风险 / 滞后」清单。<br>机器核对：`verify-design-v039.py --old 3ea78ac --new <本版>` 全 PASS，含「同一把尺子跑 `--ref 3ea78ac` 与 `--ref 790ccd6`（未落文基准）必须 FAIL」的两组反向对照（分别 FAIL 18 / 25 项）；表数 `72 → 73`、列数不一致 0、缺分隔行 0、URL 0、编号对称差为空（新旧各 196）。新 commit 续在 `3ea78ac` 之后、**恰一次提交**、快进、不覆盖、不 force-push；只触 `docs/design/MERC-3/design.md` |
| v0.3.10 | 2026-09-25 | Spec（按 Leader 处置评论 `01a0d42d` §三-A 落文，非 @Architect 新判定） | **三处代码事实更正批（MERC-20 交付回读、零轮次记账、不占评审轮次）**：① §15.2「实现落点」行的前提被代码事实推翻——`timeoutMs` 只兜住建连与认证（`SshUtil:356`）、命令执行无超时（`SshUtil:368`）⇒ 脚本耗时的护栏自此写明在命令文本上（`timeout <秒> bash <file>`），秒粒度**向上**取整（向下取整即主应用覆盖声明，触 BR-04）；② §14.14 的 holder 后缀由「步骤序号」更正为「进程内自增」（定稿 SDK 签名拿不到序号；且后缀须两路互不相同，否则 `TaskMutexManager:52` 的防御支配上 `:68` 的按值 CAS 删除会让互斥形同虚设）；③ §8.4 的 `.platform-extension/` 收口粒度细化为「删自建脚本文件 + 尝试 `rmdir`，非空即保留」。**判定正文被改写的既有行恰这三行、另有两行原段内加限定的纯插入；判定对象、编号、核对物零改动**；另登记同类相邻滞后**四处只报不改**（§1 `:47`、§15.1 首行、§7.2 B-13、§8.4 脚本注入行）交 Leader 判。单独 commit、只触本文件、快进、不 force-push。逐处核对见 §17「v0.3.10」自查表。 |
| v0.3.11 | 2026-09-25 | Spec（按 Leader 重派评论 `01a0d45d` 第 1 条落文，非 @Architect 新判定） | **四处同类相邻滞后续批（把 v0.3.10 的「只报不改」落文、零轮次记账、不占评审轮次）**：① §1 引导句与 ② §15.1 首行按代码事实更正 `timeoutMs` 的读法——该入参只流向 `SshUtil:356` 的 `getOrCreateSession`（建连与认证），命令执行 `SshUtil:368` 不带超时 ⇒ 两参重载的 `30_000L` 是建连预算、不是脚本耗时上限（§15.1 那行的取值 30 s 与出处一字未动）；③ §7.2 B-13 步骤列与 ④ §8.4「脚本注入」行的命令形状补齐为 `timeout <秒> bash <file>`，与 §15.2（v0.3.10）及已交付实现（`ExtensionScriptRunner:96`）同形（B-13 的核对物列逐字未动）。**判定正文被改写的既有行恰这四行、零新增行；判定对象、编号、核对物零改动**；另登记同族表述三处**只报不改**（§15.2「缺省值」行的「宿主机通道默认」括注、§15.3 首行、§12 RISK-D06 行——改动会牵动 §15.3 与 RISK-D06 的登记语义，超本批授权面）交 Leader 判。单独 commit、只触本文件、快进、不 force-push。逐处核对见 §17「v0.3.11」自查表。 |
| v0.3.12 | 2026-09-25 | Spec（按 Leader 派单评论 `01a0d484` §五-1 ① 落文，非 @Architect 新判定） | **同族表述三处归因更正批（把 v0.3.11 的「只报不改」落文、零轮次记账、不占评审轮次）**：① §15.2「缺省值」行 :945 那枚把 30 s 称作通道默认值的括注改指 §15.1 的**建连**预算（与 v0.3.11 更正后的标签同源）；② §15.3 首行 :956 的「不再等待」所属由 `FileAccessService` 入参改指命令文本里的远端 `timeout`，机理补为「发 SIGTERM、退出码 124、脚本可 trap、子进程可存活」（实物见交付头 `ExtensionScriptRunner` 类注释 :29-31，该类在 main 上尚不存在 ⇒ 正文只给类名、不给会漂的行号）；③ §12 RISK-D06 行 :452 第 2 格给「脚本超时」指名是哪一枚。**判定正文被改写的既有行恰这三行、零新增行；结论、取值、编号、判据、登记对象零改动**（§15.2 缺省值仍是 `600_000 ms`、§15.3 的「**不保证远端进程已被终止**」与三条子弹及「登记为 RISK-D06」一字未动、RISK-D06 的编号与缓解格未动）；同族扫描确认正文内该族旧读法自此命中 0，相邻两行（§15.2「超时后的判定」只引 ui-spec 词面、§14.13.3 本就写着 `timeout 1200`）核对为不属本族、逐字未动。另按 Leader §二 裁定给出引注的 main 侧逐条对账读数（只报不改，release / 交付头漂移只登记）。单独 commit、只触本文件、快进、不 force-push。逐处核对见 §17「v0.3.12」自查表。 |
