...
本技能为 GamePlatform 平台（PF4J + Spring 子容器 + Wujie 微前端）的插件开发与排查提供程序性知识。完整开发指南位于本 SKILL 目录下的 `references/` 分主题文件中（v3.1.0 起，原 `docs/PLUGIN_DEV_GUIDE.md` 已删除并迁移到此；当前版本 v3.8.0，见 `references/changelog.md`）。
...
| `references/scheduled_task.md` | ScheduledTaskHandler / ScheduledTaskDeclarationExtension / ScheduleService、声明式默认计划 upsert、来源隔离、cron 触发、重叠 SKIPPED、插件生命周期联动（v3.8.0 ADR-0011，独立于任务中心） | 定时任务开发 |
...
12. **定时任务体系（ADR-0011，v3.8.0）**：独立于任务中心的 cron 周期性执行体系，三张表 `scheduled_task`/`scheduled_task_run`/`scheduled_task_run_log`（与任务中心无关联）。插件两条渠道定义计划：① 声明式——`ScheduledTaskDeclarationExtension` 返回 `List<ScheduleDeclaration>`，宿主按稳定键 `pluginId:key` upsert（用户改过跳过、删过不复活）；② 编程式——注入 `ScheduleService`（自动绑定本插件 source，强制来源隔离）。`ScheduledTaskHandler` 是独立 Contract（`getKey()`/`getDisplayName()`/`getDefaultTimeoutMs()`/`execute()`），与任务中心 `TaskHandler` 完全分离：无自动重试（下一轮 cron 即天然重试）、不互斥、创建时不校验 handlerKey 触发才解析（未注册记 FAILED）。同一计划同时刻只允许一个 run（重叠记 SKIPPED）、停机不补跑。插件生命周期联动：停用暂停、热重载（purgeTasks=false）恢复、卸载移除（purgeTasks=true）物理清理。主界面「任务中心 → 定时计划」统一管理。详见 [ADR-0011](../../../docs/design/adr/0011-scheduled-task-management.md)。
...
| `ScheduledTaskHandler` | 定时任务处理器（v3.8.0 ADR-0011；`@Component` 一个 Handler，getKey/execute，无重试无互斥） | `references/scheduled_task.md` |
| `ScheduledTaskDeclarationExtension` | 声明式默认计划扩展点（`@Component`，返回 `List<ScheduleDeclaration>`，按 pluginId:key upsert） | `references/scheduled_task.md` §3 |
...
| `ScheduleService` | 定时计划编程式服务（v3.8.0 ADR-0011；注入子容器，自动绑定本插件 source，强制来源隔离） | `references/scheduled_task.md` §5 |
...
### 第 4 层：异步任务与定时任务（→ `references/task_handler.md` + `references/scheduled_task.md`）
...
| 定时计划不触发 | cron 合法性 / enabled·paused 状态 / Handler 是否注册（key 与计划 handlerKey 一致）→ `references/scheduled_task.md` §8 |
| 定时触发即 FAILED | run 的 errorMessage；handlerKey 未注册 / Handler 异常 → `references/scheduled_task.md` §8 |
...
- ADR 关联：`docs/design/adr/0001-plugin-menu-ownership.md`（菜单归属权）、`docs/design/adr/0009-platform-capability-requirements.md`（SSH 隧道 / configInfo.database / onInstanceUpdate，v3.7.0）、`docs/design/adr/0011-scheduled-task-management.md`（定时任务体系，v3.8.0）。
