...
| 双端插件 | ✅ | ✅ | 通过 `getMenus()` → 宿主拼装菜单 → Wujie 加载子应用配对（见 `references/extension-and-menus.md`） |
...
## 10. 新建插件前端的易缺失文件（v3.6.0，源自 plugin-dst 实战）

从零新建插件前端时，以下文件容易遗漏：

1. **`tsconfig.json`**：`npm run build`（vue-tsc）必需。最小可用配置：`target/module ES2020/ESNext`、`moduleResolution: "bundler"`、`noEmit: true`、`strict: true`、`paths: {"@/*": ["src/*"]}`、`types: ["node"]`。
2. **`src/env.d.ts`**（Wujie window 类型声明）：缺失则 `window.__POWERED_BY_WUJIE__` / `$wujie` / `__WUJIE_MOUNT` 全部报 TS2339：
...
3. **vite `build.outDir` 按项目布局调整**：frontend 与后端模块同仓库相邻时通常指向 `'../src/main/resources/ui'`；独立仓库布局（根目录即后端模块）时同样适用，聚合模块布局则需改为指向实际 core 模块的 `src/main/resources/ui`。
