...
> 对齐主应用: `backend/` @ 2026-09-22（分支 `release/MERC-8-extension-steps-dnf-tw`，SDK 声明层已落，主应用执行层未落）
> 关联 ADR: ADR-0001 插件菜单归属与 getMenus() 扩展点、ADR-0006 补丁安装决策树、ADR-0007 插件前端 Night Operations token 隔离、ADR-0009 平台侧能力需求、ADR-0011 定时任务管理、ADR-0014 插件开发文档三副本分工、ADR-0016 RCON 能力上提、ADR-0017 实例信息 Provider 扩展点、ADR-0029 部署扩展步骤与 dnf-tw 指定版本部署（全文见平台仓库 `docs/design/adr/`）
...
| 平台版本 | 变更 | 详见 |
|---|---|---|
| 3.11.0 | 新增 ADR-0029 部署扩展声明面（纯加法，既有实现者零改动）：`GameEnhancementExtension` 两个 default 入口 `getDeployVersions(deployType)` / `getDeployExtensionSteps(ctx)`；`extension.deploy` 包 7 个类型（`DeployVersionDeclaration` / sealed `DeployExtensionStepDeclaration` / `PatchStepDeclaration` / `ScriptStepDeclaration` / `DeployExtensionContext` / `StepKind` / `ScriptPosition`）；`PatchInstallService.installSync` + `PatchInstallProgressListener` | [sdk-reference.md](sdk-reference.md)（扩展点章 + 宿主服务面章） |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| 3.11.0 | 2026-09-22 | ADR-0029 部署扩展步骤框架的**声明层**落地（MERC-8 stage 1 / MERC-14；纯加法，`plugin-l4d2` 零改动即编译）：① `GameEnhancementExtension` 与 `getDeployConfigs()` 并列新增两个 default 入口——`getDeployVersions(String deployType)`（静态版本目录，向导步骤 2 无实例时也要能读，故签名不含 instanceId）与 `getDeployExtensionSteps(DeployExtensionContext ctx)`（按实例配置动态算步骤集），均返回空集合。② 新包 `com.gameplatform.plugin.extension.deploy` 七个类型：4 record（`DeployVersionDeclaration` / `PatchStepDeclaration` / `ScriptStepDeclaration` / `DeployExtensionContext`）+ 1 sealed interface（`DeployExtensionStepDeclaration`，`permits` 那两个步骤 record，带 `kind()` 分派位）+ 2 enum（`StepKind` / `ScriptPosition`）；sealed 的 `permits` 要求同包 ⇒ 全部落在一包内，SDK 层零游戏语义。补丁步骤与脚本步骤是**两个类型而非一个带判别位的大杂烩**：两套必填与「正文 / URL 二选一」在同一个构造器上无法用类型表达，能编译期挡住的非法组合不留到运行期。③ `PatchInstallService` 新增 default `installSync(request, listener)` + `PatchInstallProgressListener`（三方法照宿主执行器回调形状）；`install()` 签名与行为一字未动，其注释按 RISK-D05 标出「异步提交路径不透传 `includePattern` / `headers`，需要它们请用 `installSync`」。④ 声明侧的坑：`fatal` 是原始 `boolean`，PRD 口径缺省致命而 Java 缺省是 `false` ⇒ 声明必须显式写 `true`；`patches` 拼接 `scripts` 必须带显式类型见证，不带则 javac 17 报 `List<INT#1>无法转换为List<DeployExtensionStepDeclaration>`。⑤ **本期不含**主应用侧执行与校验（版本目录服务、EXTENSION 阶段、`installSync` 的 core 实现、前端呈现均在后续里程碑），也**不新增** `game-platform.*` / `plugin.*` 配置开关、零 schema 变更。附带验收资产 `backend/plugin-stub`（最小桩插件 + 受控夹具 + 桩游戏元数据，只在 `-Pacceptance-assets` profile 下构建，不进默认 `<modules>`、不进产品发布物） |
