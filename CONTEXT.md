...
### 开图命令（Map Launch Command）
启动三方地图某个章节的服务器控制台命令（`map <章节码>`）。源自地图分享页作者标注，按战役组织、多章多命令。
_Avoid_: 建图命令（源站/页面展示用词，领域内不用）、建图（易与"制作地图"混淆）

### 爬取水位线（Crawl Watermark）
增量爬取判断"条目是否需要重新抓取"的时间基准：库内已有条目列表更新时间的最大值回拨安全冗余天数。来源列表按更新时间倒序排列时，整页旧于水位线即可停止翻页。

### 地图识别（Map Recognition）
在主机侧解析三方地图 VPK 得到战役名、章节与模式的过程。结果按 VPK 内容摘要键控、插件级共享（跨实例复用）：同一文件全平台只识别一次。实例维度只维护"文件 → 摘要"的索引。
_Avoid_: 地图解析（解析是识别的执行动作，不单独成词）

### 识别摘要（Recognition Digest）
VPK 文件的内容哈希，识别记录的主键。内容相同即视为同一地图，覆盖安装产生新摘要、自然失效旧记录，无需显式比对文件。
...
### 部署扩展步骤（Deploy Extension Step）
部署流程中"部署动作完成、实例正式启动前"执行的一段声明式自动化：补丁替换或脚本执行。由游戏插件声明步骤清单（来源 + 目标路径 / 脚本内容或来源 / 执行位置），主应用部署管线按序**阻塞执行**，每步呈现于部署日志——扩展不是插件自由发挥的钩子，而是主应用执行的声明式管线。

### 部署扩展阶段（Deploy Extension Stage）
部署流水线新增的显式阶段，位于"执行部署（DEPLOY）"与"健康检查（HEALTH_CHECK）"之间。对"部署即起容器"的适配器，先停实例、执行扩展步骤、再进入既有启动与健康检查，保证实例**第一次启动即目标版本**，消除"先起错版本再重启"的中间态。

### 步骤致命性（Step Fatality）
步骤级声明：致命（默认）——任一步失败即部署失败、实例 ERROR、不交付；非致命——仅记日志，部署照常完成，留给"可选优化类脚本"。

### 版本目录（Version Catalog）
游戏侧可选择的版本清单：`版号 → 镜像 tag（可选，缺省沿用默认）+ 补丁集 + 脚本集 + 目标路径`。由插件经 ADR-0008 声明体系的类型化入口 `getDeployVersions` 声明（该体系的读时合并只作用于配置读取路径、不作用于部署执行路径，故目录不走 `getDeployConfigs()`，见 ADR-0029 决策 9 注）；用户在部署向导中选择，选择结果随实例配置（configInfo）落库，重部署按同一配方重放。dnf-tw 现状：上游镜像只提供默认版本，其余版本靠补丁 + 脚本"改"出来。
